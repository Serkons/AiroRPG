...
dungeon.enter {
  ...
  if ($get.level($1) > $readini($txtfile(battle2.txt), DungeonInfo, DungeonLevel)) { $levelsync($1, $readini($txtfile(Battle2.txt), DungeonInfo, DungeonLevel)) }
...
dungeon.begin {
  ...
  if ((%number.of.players = 0) || (%number.of.players < $readini($txtfile(battle2.txt), DungeonInfo, PlayersNeeded))) { $display.message($readini(translation.dat, errors, NotEnoughPartyMembersForDungeon), global) | unset %random.chance1 | $clear_battle | halt }
...
dungeon.firstroom {
  ...
  set %dungfile $readini($txtfile(battle2.txt), DungeonInfo, DungeonFile)
  $display.message(02* $readini($dungeonfile($dungeon.dungeonfile), $dungeon.currentroom, desc), battle)
...
dungeon.rewardorbs {
  ...
  if (($1 != dungeonclear) && ($1 != failure)) { $display.message($readini(translation.dat, battle, RewardOrbsWin), battle) | $display.message($readini(translation.dat, battle, RewardKillCoinsWin), battle)  }
}
...
dungeon.end { 
  ...
  if ($1 = failure) { 
    ...
    remove $dungeonfile(CreatedDungeon)
...
dungeon.nextroom {
  ...
  if ($readini($dungeonfile($dungeon.dungeonfile), $dungeon.currentroom, ChestRoom) = true) { $dungeon.chestroom }
...
dungeon.restoreroom {
  ...
  $display.message(2[Room: 8 $+ RestoreRoom $+ 2] 07* The party feels their health $+ $chr(44)  tp $+ $chr(44)  and power restored in this room)
...
dungeon.chestroom {
  ...
  var %nu $rand(1,2)
  if (%nu == 2) { $give_armor_chest }
  if (%nu == 1) {
    ; this room get a random item from a chest to one player or not
    set %chest.winners $readini($txtfile(battle2.txt), Dungeon, List)
    set %chest.winner $gettok(%chest.winners, $rand(1, $numtok(%chest.winner,46)), 46)
    set %cflag $readini($char(%chest.winner), info, flag)
    if (%cflag != npc) || (%cflag != monster) { 
      set %chests blue.brown.gobbiebox.black.gold.green.orange.purple.rainbow.silver
      set %random.color $gettok(%chests, $rand(1,$numtok(%chests,46)), 46)
      set %chestfile chest_ $+ %random.color $+ .lst
      set %chest.name $lstfile(chest_ $+ %random.color $+ .lst)
      set %chest.contents $read(%chest.name)
      set %total.item1 $readini($char(%chest.winner), Item_Amount, %chest.contents)
      if (%total.item1 == $null) { writeini $char(%chest.winner) Item_Amount %chest.contents 1 }
      else { inc %total.item1 1 | writeini $char(%chest.winner) Item_Amount %chest.contents %total.item1 }
      if (%cflag == npc) || (%cflag == monster) { return }
      var %current.room $dungeon.currentroom
      inc %current.room 1
      writeini $txtfile(battle2.txt) DungeonInfo CurrentRoom %current.room

      if ($dungeon.dungeonovercheck = true) { $dungeon.end | halt } 
      var %mess2 3 $+ %chest.winner  gets the new item from the %random.color chest:  %chest.contents
