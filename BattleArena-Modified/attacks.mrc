...
ON 50:TEXT:*normalattack *:*:{ 
  if ($2 != normalattack) { halt } 
  else { 
...
alias CriticalCut_check {
  if (%counterattack = on) { return }
  if (%attack.damage = 0) { return }

  if ($readini($char($1), skills, CriticalCut) > 0) {
    ...
    set %bleed.chance $rand(1,100)
    set %CriticalCut.chance $calc($readini($char($1), skills, CriticalCut) * 20)

    if ($augment.check($1, EnhanceCriticalCut) = true) { inc %CriticalCut.chance %augment.strength  } 

    if (%bleed.chance <= %CriticalCut.chance) {
      writeini $char($2) status bleed yes | $set_chr_name($2) | set %statusmessage.display 04 $+ %real.name suffered a deep cut and is now bleeding to death!
    }
  }
}
