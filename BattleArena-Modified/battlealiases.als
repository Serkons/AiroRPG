...
status.effects.turns {
  var %negative.status.effects confuse.poison.curse.cocoon.silence.weaponlock.drunk.zombie.doll.virus.slow.defensedown.strdown.intdown.charm.paralysis.bored.mini.bleed

  if ($istok(%negative.status.effects,$1,46) = $true) { 
    if ($return_playersinbattle <= 1) && ($1 != bleed) { return 2 }
    if ($return_playersinbattle <= 1) && ($1 = bleed) { return 5 }
    if ($1 = doom) { return 4 }
...
check_for_double_turn {  $set_chr_name($1)
  ...
  if ($readini($char($1), battle, hp) == 1) && ($readini($char($1), Monster, Ispoke) == true) && ($readini($char($1), Status, cocoon) == no) { $spawn_after_death($1) | $clear_negative_status($1) | writeini $char($1) battle hp 0 | $next | halt }
  if ($readini($char($1), battle, hp) <= 0) && ($readini($char($1), Monster, Ispoke) != true) { $next | halt }
  if ($readini($char($1), battle, hp) <= 0) && ($readini($char($1), Monster, Ispoke) == true) { $next | halt }
...
boost_monster_stats {
  ...
  if ($2 = evolve) { 
    if ($isfile($boss($1)) = $false) { inc %monster.level $rand(12,20) }
    if ($isfile($boss($1)) = $true) { inc %monster.level $rand(20,40)  }
  }

  if ($2 = evolve-poke) { 
    if ($isfile($boss($1)) = $false) { inc %monster.level $rand(15,25) }
    if ($isfile($boss($1)) = $true) { inc %monster.level $rand(25,45)  }
  }
...
boost_monster_hp {
  ...
  if (%powercrystal = on) {
    if (%powercrystal.squad < 3) { var %hp.modifier 1.7 }  
    if ((%powercrystal.squad >= 3) && (%besieged.squad < 5)) { var %hp.modifier 2.2 }
    if (%powercrystal.squad >= 5) { var %hp.modifier 2.77 }
  }
...
deal_damage {
  ...
  if ($person_in_mech($2) = false) { 
    if ($readini($char($2), battle, HP) <= 0) { 
      ...
      unset %times.to.defeat.flag
      var %times.to.defeat $readini($char($2), Info, TimesToDefeat)
      if (%times.to.defeat = $null) { var %times.to.defeat 1 }
      dec %times.to.defeat 1
      writeini $char($2) Info TimesToDefeat %times.to.defeat

      if (%times.to.defeat >= 1) { 
        ; The monster still needs to be defeated before it will officially die.
        set %times.to.defeat.flag true
        writeini $char($2) Battle HP $readini($char($2), Battle, BaseStats, HP)
        writeini $char($2) Battle Status alive      
      }

      if (%times.to.defeat <= 0) {    
...
display_damage {
  ...
  if ($3 = weapon) { 
    ...
    if (%counterattack != shield) {
      ...
      if (%counterattack = on) { 
        ...
        $set_chr_name($1) | set %enemy %real.name | set %target $1 | set %attacker $2 | $set_chr_name($2) | set %file.name $1 | set %user %real.name 
      }
  ...
  if ($3 = tech) {
    if (%showed.tech.desc != true) { 
      ...
      $display.message(03 $+ %user $+  uses tech 11 $4 3, battle)
  ...
  if ($3 != limitbreak) { 
    if (%number.of.hits > 1) { var %damage.message The attack hits04 %number.of.hits times and does %damage.display.color $+  $+ $bytes(%attack.damage,b) total damage to %enemy $+ .  %enemy has also 7 $readini($char(%file.name), Battle, Hp)  hp left %style.rating }
    else { var %damage.message The attack does %damage.display.color $+  $+ $bytes(%attack.damage,b) damage to %enemy $+ . %enemy has also 7 $readini($char(%file.name), Battle, Hp)  hp left %style.rating }
  }
  if ($3 = limitbreak) { var %damage.message The limit break does %damage.display.color $+  $+ $bytes(%attack.damage,b) damage 3to %enemy $+ .  %enemy has also 7 $readini($char(%file.name), Battle, Hp)  hp left %style.rating 
  }
  ...
  if ($readini($char(%target), battle, HP) <= 0) { 
    ...
    if ($readini($char(%target), info, flag) != $null) {  $random.healing.orb($1,%target) | $randoms.redorb.reward($1,%target)  }

    ;::::::::::check for a drop an then drops it to the killer

    if ($readini($char(%target), Stuff, Drops) != $null) {
      set %drop.chance.item $rand(1,100)
      if (%drop.chance.item <= 25) {
        var %drops $readini($char(%target), Stuff, Drops)
        var %numdrops $numtok(%drops, 46)
        var %get.item $gettok(%drops, $rand(1, %numdrops), 46)
        var %item.total $readini($char($1), item_amount, %get.item)
        if (%item.total == $null) { writeini $char($1) Item_Amount %get.item 1 }
        else { inc %item.total 1 | writeini $char($1) Item_Amount %get.item %item.total }
        var %mess 12 $1 gets the item %get.item from %real.name  
  ...
  if (($readini($char(%target), battle, HP) > 0) && ($person_in_mech($2) = false)) {
    ...
    if ($3 = tech) { $renkei.check($1, %target) }
    ...
    ; If the target was defeated once, but is revived show a message..
    if (%times.to.defeat.flag = true) { $display.message($readini(translation.dat, battle, DefeatedMonsterStandsBackUp), battle) }
...
display_heal {
  ...
  if ($3 = tech) {
    if (%showed.tech.desc != true) {
      $set_chr_name($1)
      $display.message(03 $+ %real.name $+  uses tech 11 $4 3, battle)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; This Function checks and give a small
; amount of orbs when killing
; a monster
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
randoms.redorb.reward {
  if ($person_in_mech($1) = true) { return }
  if (%battle.type = ai) { return }
  ; check to see if Some random Red orb appears.
  if ($1 != $2) {
    set %flag1 $readini($char($1), Info, Flag)
    if (%flag1 == monster) || (%flag1 == npc) { return }
    var %orbs.get $rand(1,20)
    var %currentorbs $readini($char($1), Stuff, RedOrbs)
    var %neworbs $round($calc(%currentorbs + %orbs.get), 0)
    writeini $char($1) Stuff RedOrbs %neworbs
    var %message $chr(3) $+ 3 $+ $set_chr_name($2) $+ %real.name drops a small OrbPot that gives  %orbs.get orbs to %user $+ !
    $display.message(%message, battle)
  }
}
...
taunt {
  ...
  if ($readini($char($2), info, HealByTaunt) = true) { 
    ...
    if (%taunt.effect = 1) { var %taunt.str $readini($char($2), battle, str) | inc %taunt.str $rand(1,12) | writeini $char($2) battle str %taunt.str | $set_chr_name($2) | $display.message($readini(translation.dat, battle, TauntRage), battle) }
    if (%taunt.effect = 2) { var %taunt.def $readini($char($2), battle, def) | inc %taunt.def $rand(1,12) | writeini $char($2) battle def %taunt.def | $set_chr_name($2) | $display.message($readini(translation.dat, battle, TauntDefensive), battle) }
    ...
    if (%taunt.effect = 6) { $restore_hp($2, $rand(1,110)) | $set_chr_name($2) |  $display.message($readini(translation.dat, battle, TauntLaugh), battle) | unset %taunt.hp }
...
random.weather.pick {
  ...
  var %mess 2====7 Weatherinformation 2====
  if (%weather.line = $null) { $display.message(%mess, battle) |  $display.message(10The weather changes.  It is now %new.weather, battle) }
...
random.playersgofirst {
  ...
  if (%pcrystalrun = on) { var %playersfirst.chance 1 }
...
character.revive {
  ...
  writeini $char($1) Status Bleed no | writeini $char($1) Status HeavyBleed no | writeini $char($1) Status HeavyBleed no
  ...
  writeini $char($1) Status bleed.timer 1
...
inflict_status {
  ...
  if ($3 = random) { 
    var %random.status.type $rand(1,21)
    ...
    if (%random.status.type = 21) { set %status.type bleed | var %status.grammar bleed to death }
  ...
  if ($3 = bleed) { set %status.type bleed | var %status.grammar bleed to death }
  ...
  if ((%resist.skill < 100) || (%resist.skill = $null)) {
    ...
    if (%chance > 45) { $set_chr_name($2) 
      ...
      if (%status.type = bleed) && ($readini($char($2), status, bleed) = yes) { writeini $char($2) status bleed no | writeini $char($2) HeavyBleed yes | writeini $char($2) status bleed.timer %enfeeble.timer }
      if (%status.type = bleed) && ($readini($char($2), Status, HevyBleed) != yes) { writeni $char($2) status bleed yes | writeini $char($2) status bleed.timer %enfeeble.timer }
      ...
      if ((((((%status.type != doom) && (%status.type != poison) && (%status.type != charm) && (%status.type != petrify) && (%status.type != bleed) && (%status.type != removeboost)))))) { writeini $char($2) Status %status.type yes | writeini $char($2) status %status.type $+ .timer %enfeeble.timer   }
...
self.inflict_status {
  ...
  if (%self.status.type = random) { 
    ...
    if (%random.status.type = 17) { set %self.status.type bleed }
  }
...
do.self.inflict.status {
  ...
  if (%self.status.type = bleed) { set %status.type bleed | var %status.grammar bleed to death }
  ...
  ;; Bleeding
  if (%status.type = bleed) && ($readini($char(%inflict.user), status, bleed) = yes) { writeini $char(%inflict.user) status bleed no | writeini $char(%inflict.user) status HeavyBleed yes | writeini $char(%inflict.user) status bleed.timer %enfeeble.timer }
  if (%status.type = bleed) && ($readini($char(%inflict.user), status, HeavyBleed) != yes) { writeini $char(%inflict.user) status bleed yes | writeini $char(%inflict.user) status bleed.timer %enfeeble.timer }
  ;;; Bleeding
  ...
  if ((((%status.type != poison) && (%status.type != bleed) && (%status.type != charm) && (%status.type != petrify)))) { writeini $char(%inflict.user) Status %status.type yes | writeini $char(%inflict.user) status %status.type $+ .timer %enfeeble.timer   }
}
...
trickster_dodge_check {
  ...
  if (%attacker.speed > %target.speed) { inc %dodge.chance $rand(5,15) } 

  ; If the target is 'mini' then increase the dodge chance by 10%
  if ($is_mini($1) = true) { inc %dodge.chance 10 }

  if ((%battle.type = torment) && ($readini($char($1), info, flag) = $null)) { inc %dodge.chance $rand(10,20) }
  if ((%battle.type = cosmic) && ($readini($char($1), info, flag) = $null)) { inc %dodge.chance $rand(10,20) }
  if ((%battle.type = dungeon) && ($readini($char($1), info, flag) = $null)) { inc %dodge.chance $rand(5,15) }
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
multiple_wave_check {
  if (%besieged != on) || (%powercrystal != on) { 
    ...
    if (%pcrystalrun = on) { return }
  }

  unset %number.of.monsters.needed

  if ((((((%besieged != on) && (%powercrystal != on) && (%battle.type != defendoutpost) && (%battle.type != assault) && (%battle.type != torment) && (%mode.gauntlet != on)))))) {  
  ...
  if ((%powercrystal = on) && (%powercrystal.squad <= 5)) { var %random.wave.chance 2 }
  if ((%powercrystal = on) && (%powercrystal.squad > 5)) { $endbattle(victory) | halt }
  ...
  ; PowerCrystal type
  if (%powercrystal = on) {
    inc %powercrystal .squad 1
    $generate_monster(powercrystal)

    if (%powercrystal.squad < 5) { $display.message(04The next Squad of Super Elite Monsters has arrived!,battle) }
    if (%powercrystal.squad = 5) { $display.message(04The final Squad Super Elite Monsters has arrived!,battle) }
...
spawn_after_death {
  ...
  while (%value <= %number.of.monsters) { 
    ...
    ; Boost the monster if its a Evolved Pokemon version

    if ($readini($char(%current.monster.to.spawn), Monster, IsPoke) == true) && ($readini($char(%current.monster.to.spawn), Monster, IsEvolved) == true) {
      $boost_monster_stats(%current.monster.to.spawn, evolve-poke) 
...
battlefield.object {
  ...
  ;if (%object.chance <= 25) {
  var %n $numtok($readini($dbfile(battlefields.db), %current.battlefield, objects), 46)
  while (%n >= 1) {
    var %breakable.objects $readini($dbfile(battlefields.db), %current.battlefield, objects)
    var %object.chosen $gettok(%breakable.objects, %n, 46)
      ...
      if (($readini($char(%who.battle), battle, status) = dead) || ($readini($char(%who.battle), monster, type) = object)) { inc %battletxt.current.line }
        ...
        if (%event.status.type != $null) { 
          if ($person_in_mech(%who.battle) = false)  { 
            var %mess 2====7 Battlefield Event: 4 Status2====
            $display.message(%mess, battle)

            writeini $char(%who.battle) status %event.status.type yes
          }
        }
        if ($readini($dbfile(battlefields.db), %current.battlefield, event $+ %battlefield.event.number) = damage) { 
          ...
          var %mess 2====7 Battlefield Event: 4 Damage2====
          $display.message(%mess, battle)
        ...
        if ($readini($dbfile(battlefields.db), %current.battlefield, event $+ %battlefield.event.number) = heal) {
          ...
          var %mess 2====7 Battlefield Event: 4 Heal2====
          $display.message(%mess, battle)
...
modifer_adjust {
  ...
  if (($readini($char($1), modifiers, ResistMelee) >= 1) && ($3 = melee)) { 
    ...
    if (%attack.damage <= 0) { set %attack.damage 0 }
    var %mon1 $char($1)
    var %mon $remove(%mon1, " $+ $mircdir $+ characters\, .char $+ ")
    $display.message(3Sorry but  %mon  is resist to melee attacks, battle)
  }
}
...
add.monster.drop {
  ...
  if (%dropchance2 <= %random.chance.reward) { 
...
add.monster.drop2 {
  ...
  var %drop.items $readini($char($2), Stuff, Drops)
  ...
  var %random.chance.reward 40
  ;set %random.chance.reward $readini($char($2), Stuff, DropChance)
  set %dropchance3 $rand(1,100)
  ...
  if (%dropchance3 <= %random.chance.reward) { 
    var %drop.reward $gettok(%drop.items,$rand(1,$numtok(%drop.items, 46)),46)
    var %temp.player.list $readini($txtfile(battle2.txt), drops, $1)
    var %temp.player.list $addtok(%temp.player.list, %drop.reward, 46)
...
powercrystal.reward {  
  unset %powercrystal.drop.rewards

  var %powercrystal.reward.amount 1
  ...
  while (%battletxt.current.line <= %battletxt.lines) { 
    ...
    if ((%flag = monster) || (%flag = npc)) { inc %battletxt.current.line 1 }
      ...
      if (%player.amount = $null) { var %player.amount 0 }
      inc %player.amount %powercrystal.reward.amount
      writeini $char(%who.battle) item_amount AlliedLockBox %player.amount
      %powercrystal.drop.rewards = $addtok(%powercrystal.drop.rewards,  $+ $get_chr_name(%who.battle) $+  $+ $chr(91) $+ AlliedLockBox x $+ %powercrystal.reward.amount  $+  $+ $chr(93) $+ , 46)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; Displays PowerCrystal rewards
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
show.powercrystal.reward {
  if (%powercrystal.drop.rewards = $null) { return }
  var %replacechar $chr(044) $chr(032)
  %powercrystal.drop.rewards = $replace(%powercrystal.drop.rewards, $chr(046), %replacechar)
  $display.message($readini(translation.dat, battle, powercrystal.DropWin),battle) 
  unset %item.drop.rewards | unset %powercrystal.drop.rewards
}
...
poison_check { 
  ...
  if (($readini($char($1), status, poison) = yes) || ($readini($char($1), status, HeavyPoison) = yes)) {
    ...
    if (%poison.timer < $status.effects.turns(poison)) {
      ...
      if (%poison >= %hp) { $player.status($1) | set %status.message $readini(translation.dat, battle, TurnMessage) | set %status.message2 $readini(translation.dat, battle, TurnMessage2) | set %status.message3 $readini(translation.dat, battle, TurnMessage3) | set %status.message4 $readini(translation.dat, battle, TurnMessage4) |  $display.message(%status.message, battle) | $display.message(%status.message2, battle) | $display.message(%status.message3, battle) | $display.message(%status.message4, battle) | $set_chr_name($1) | $display.message($readini(translation.dat, status, PoisonKills), battle) | writeini $char($1) Battle HP 0 | writeini $char($1) Battle Status Dead |  $increase.death.tally($1)  | $add.style.effectdeath | $check.clone.death($1)
...
;;;;;;;;;;;;;;;;;;;;;;
This Fuction checks for Bleeding
;;;;;;;;;;;;;;;;

bleed_check { 
  set %debug.location bleed_check
  ;  Check for the an accessory that bleeding the user
  if ($accessory.check($1, IncreaseMeleeAddBleed) = true) {
    writeini $char($1) status bleed yes
    writeini $char($1) status bleed.timer 0
    unset %accessory.amount
  }

  if (($readini($char($1), status, bleed) = yes) || ($readini($char($1), status, HeavyBleed) = yes)) {
    set %bleed.timer $readini($char($1), status, bleed.timer)  
    if (%bleed.timer >= $status.effects.turns(bleed)) {  
      writeini $char($1) status bleed no
      writeini $char($1) status HeavyBleed no 
      writeini $char($1) status bleed.timer 0
      $set_chr_name($1) | write $txtfile(temp_status.txt) $readini(translation.dat, status, BleedWornOff) 
      return 
    }
    if (%bleed.timer < $status.effects.turns(bleed)) {
      ...
      $goldorb_check($1,status) | $spawn_after_death($1) | remini $char($1) Renkei | next | halt }
      if (%bleed < %hp) {
        $set_chr_name($1) | write $txtfile(temp_status.txt) $readini(translation.dat, status, BleedMessage) | dec %hp %bleed | writeini $char($1) Battle HP %hp |  unset %hp | unset %bleed
...
HeavyBleed { 
  ...
  $goldorb_check($1, status) | $spawn_after_death($1) | remini $char($1) Renkei | next | halt }
  if (%bleed < %hp) { $set_chr_name($1) | write $txtfile(temp_status.txt) $readini(translation.dat, status, BleedMessage) | dec %hp %bleed | writeini $char($1) Battle HP %hp | unset %hp | unset %bleed 
...
HeavyPoison { 
  ...
  set %hp $readini($char($1), Battle, HP) | $set_chr_name($1)
  unset %max.hp
  if (%poison >= %hp) { $player.status($1) | set %status.message $readini(translation.dat, battle, TurnMessage) | set %status.message2 $readini(translation.dat, battle, TurnMessage2) | set %status.message3 $readini(translation.dat, battle, TurnMessage3) | set %status.message4 $readini(translation.dat, battle, TurnMessage4) |  $display.message(%status.message, battle) | $display.message(%status.message2, battle) | $display.message(%status.message3, battle) | $display.message(%status.message4, battle) | $display.message($readini(translation.dat, status, PoisonKills), battle) | writeini $char($1) Battle HP 0 | writeini $char($1) Battle Status Dead | $increase.death.tally($1) | $add.style.effectdeath | $check.clone.death($1)
  $goldorb_check($1, status) | $spawn_after_death($1) | remini $char($1) Renkei | next | halt }
  if (%poison < %hp) { $set_chr_name($1) | write $txtfile(temp_status.txt) $readini(translation.dat, status, PoisonMessage) | dec %hp %poison | writeini $char($1) Battle HP %hp | unset %hp | unset %bleed 
...
cocoon_check1 { 
  ...
  if (%cocoon.timer < $status.effects.turns(cocoon)) { 
    ...
    if ($readini($char($1), Status, cocoon) = yes) {   
      ...
      if ($readini($char($1), Monster, IsPoke) = true) {
        $set_chr_name($1)
        writeini $char($1) status cocoon no | writeini $char($1) status cocoon.timer 0
        ;set %curbat $readini($txtfile(battle2.txt), Battle, List) | %curbat = $remtok(%curbat, %oldmon, 46) |  %curbat = $addtok(%curbat, %newmon, 46) | writeini $txtfile(battle2.txt) Battle List %curbat | write $txtfile(battle.txt) %newmon | write -ds $+ %oldmon $txtfile(battle.txt) | writeini $txtfile(battle2.txt) Battle List %curbat
        ;end of Pokemon     
...
cocoon_check { 
  ...
  if (%cocoon.timer < $status.effects.turns(cocoon)) { 
    ...
    if ($readini($char($1), Status, cocoon) = yes) && ($readini($char($1), Monster, IsPoke) == true) {   
      if ($readini($char($1), descriptions, EvolveEnd) = $null) { set %skill.description $readini(translation.dat, status, CocoonWornOff) }
      else { set %skill.description $readini($char($1), descriptions, EvolveEnd) }
      $set_chr_name($1) | write $txtfile(temp_status.txt) 12 $+ %real.name  $+ %skill.description
      writeini $char($1) status cocoon no | writeini $char($1) status cocoon.timer 0
      writeini $char($1) Battle Hp 1
      unset %cocoon.timer | unset %skill.description
      ;$boost_monster_stats($1, evolve-poke) | ;$fulls($1) | return | $next
    }
    if ($readini($char($1), Status, cocoon) = yes) && ($readini($char($1), Monster, IsPoke) != true) {  
      if ($readini($char($1), descriptions, EvolveEnd) = $null) { set %skill.description $readini(translation.dat, status, CocoonWornOff) }
      else { set %skill.description $readini($char($1), descriptions, EvolveEnd) }
