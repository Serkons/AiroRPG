...
set_chr_name {
  ...
  set %file $char($1)
  set %file.name $remove(%file, " $+ $mircdir $+ characters\, .char $+ ")
...
get.level {
  ...
  if ($readini($char($1), info, flag) = Boss) {  var %level $floor($calc(%level * %weight)) }
...
get.level.statweight {
  ...
  if ($readini($char($1), info, flag) = monster) { 
    ...
    if (%powercrystal = on) { return %weight }
    if (%pcrystalrun = on) { return %weight }
    if (%hq-gen = on) { return %weight) }
  ...
  if ($readini($char($1), Battle, Status) = dead) { set %all_status dead | return } 
    ...
    while (%value <= %status.lines) {
      ...
      if (($readini($char($1), Status, %current.status) = yes) || ($readini($char($1), Status, %current.status) = on)) { 
        ...
        if ((%current.status = bleed) && ($readini($char($1), status, heavybleed) = yes)) { $status_message_check($readini(translation.dat, statuses, HeavyBleed)) }
...
readskills2 {
  $checkchar2($1) | $skills.list2($1) | $set_chr_name($1) 
...
readtamingitems {
  if (%taming.items.list != $null) {
    if ($2 = channel) { $display.message($readini(translation.dat, system, ViewTamingItems),private) | if (%taming.items.list2 != $null) { $display.message( $+ %taming.items.list2,private) } |  if (%taming.items.list3 != $null) { $display.message( $+ %taming.items.list3,private) } | if (%taming.items.list4 != $null) { $display.message( $+ %taming.items.list4,private) } | if (%taming.items.list5 != $null) { $display.message( $+ %taming.items.list5,private) } | if (%taming.items.list6 != $null) { $display.message( $+ %taming.items.list6,private) } }
    if ($2 = private) { $display.private.message($readini(translation.dat, system, ViewTamingItems)) | if (%taming.items.list2 != $null) { $display.private.message(05 $+ %taming.items.list2) } | if (%taming.items.list3 != $null) { $display.private.message( $+ %taming.items.list3) } | if (%taming.items.list4 != $null) { $display.private.message( $+ %taming.items.list4) } | if (%taming.items.list5 != $null) { $display.private.message( $+ %taming.items.list5) } | if (%taming.items.list6 != $null) { $display.private.message( $+ %taming.items.list6) } }
    if ($2 = dcc) { $dcc.private.message($nick, $readini(translation.dat, system, ViewTamingItems)) | if (%taming.items.list2 != $null) {  $dcc.private.message($nick,  $+ %taming.items.list2) } | if (%taming.items.list3 != $null) { $dcc.private.message($nick,  $+ %taming.items.list3) } | if (%taming.items.list4 != $null) { $dcc.private.message($nick,  $+ %taming.items.list4) } |  if (%taming.items.list5 != $null) { $dcc.private.message($nick,  $+ %taming.items.list5) } |  if (%taming.items.list6 != $null) { $dcc.private.message($nick,  $+ %taming.items.list6) } }
  }

  if (%taming.items.list = $null) { 
    if ($2 = channel) { $display.message($readini(translation.dat, system, HasNoTamingItems),private) }
    if ($2 = private) { $display.private.message($readini(translation.dat, system, HasNoTamingItems)) }
    if ($2 = dcc) {  $dcc.private.message($nick, $readini(translation.dat, system, HasNoTamingItems)) }
  }    

  unset %tamingitems.items.*
}
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
character.stat.levelup.check {
  ...
  if (%stat.xp = 10) { 
    ...
    var %basestat $readini($char($1), BaseStats, $2)
    inc %basestat 1
    writeini $char($1) Battle $2 %basestat
    writeini $char($1) BaseStats $2 %basestat
    msg #AiroRPG Congratulation $1 your $2 was increased by +1. Nice Work :)
  }
