...
on 2:Chat:!info*: { $view-info($1, $2, $3, $4) }
on 2:Chat:!help*: { $gamehelp($2, $nick) }
on 2:Chat:!ishop*: { $shop.start($1, $2, $3, $4, $5) }
on 2:Chat:!sell*: { $shop.start(!ishop, sell, $2, $3, $4, $5) }
...
on 2:Chat:!The conquest*: { $conquest.display }
...
ON 2:Chat:!mycdesc *: { $checkscript($2-)  | writeini $char($nick) Descriptions Char $2- | $okdesc($nick , Character) }
ON 2:Chat:!myskill desc *: { $checkscript($3-)  | $set_chr_name($nick) 
...
ON 2:Chat:!setmygender *: { $checkscript($2-)
...
on 2:Chat:!mystats*: { unset %all_status 
  if ($2 = $null) { 
...
on 2:Chat:!weapons*: { unset %*.wpn.list | unset %weapon.list
  ...
  /.timerDisplayWeaponList $+ $nick -d 1 1 /display_weapon_lists %wpn.lst.target dcc $nick
}
...
on 2:Chat:!shields*: { unset %*.shld.list | unset %shield.list
  ...
  /.timerDisplayshieldList $+ $nick -d 1 1 /display_shield_lists %shld.lst.target dcc $nick
}
...
on 2:Chat:!skills*: {
  ...
  if ($2 = equip) { $skill.equip($nick, $3, dcc) | halt }
  if (($2 = unequip) || ($2 = remove)) { $skill.unequip($nick, $3, dcc) | halt }
  if ($2 = equipped) { 
    if ($3 = $null) { $skill.equipped.list($nick, dcc) | halt }
    else { $skill.equipped.list($3, dcc) | halt }
  }
...
on 2:Chat:!ammo*: {
  if ($2 != $null) { $checkchar($2) | $ammo.list($2) | $set_chr_name($2) | $readammo($2, dcc) }
  else {  $ammo.list($nick) | $set_chr_name($nick) | $readammo($nick, dcc) }
}
...
on 2:Chat:!Mystyle*: { unset %*.style.list | unset %style.list
  if ($2 = $null) { 
...
on 2:Chat:!Show dragonballs*: { $db.display($nick) }

on 2:Chat:!Go dragon hunt*: { $character.dragonhunt($nick) } 
on 2:Chat:!The dragon list*: {  $dragonhunt.listdragons } 

on 2:Chat:!bounty*: { $bounty.display }

on 2:Chat:ACTION throw *: {
...
on 2:Chat:!save difficulty*: {  $set_chr_name($nick) | $checkchar($nick) 
  ...
  if ($3 > 200) { $display.message($readini(translation.dat, errors, DifficultyCan'tBeOver200),private) | halt }
...
on 2:Chat:!forging*: { $reforge.weapon($nick, $2) }
on 2:Chat:!augment*: {
...
ON 2:Chat:!used*: {  unset %real.name | unset %enemy
  if ($is_charmed($nick) = true) { $set_chr_name($nick) | $dcc.private.message($nick, $readini(translation.dat, status, CurrentlyCharmed)) | halt }
  if ($is_confused($nick) = true) { $set_chr_name($nick) | $dcc.private.message($nick, $readini(translation.dat, status, CurrentlyConfused)) | halt }
...
ON 2:Chat:!warp*: { $skill.warp($nick, $2-) }
ON 2:Chat:!lockpicking*: { $skill.lockpicking($nick) }
