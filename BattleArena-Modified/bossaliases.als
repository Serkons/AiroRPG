...
get_boss_type {
  ...
  if (%pcrystalrun = on) { set %boss.type normal | return } 
  ...
  var %enable.zombies $readini(system.dat, system, EnableZombies)
  ...
  ;if (%winning.streak.check < 85) { var %enable.zombies false }
  ...
  if ((%enable.zombies = true) && (%boss.chance <= 45)) { %boss.choices = %boss.choices $+ .zombies }
  ...
  if (%enable.zombies = true) {
    if ((%winning.streak.check >= 15) && (%winning.streak.check <= 1500)) { %boss.choices = %boss.choices $+ .zombies }
  }
...
generate_allied_guard {
  ...
  set %monster.name Allied_Guard | set %monster.realname Allied Crystal Guard
  ...
  if (%current.battlestreak < 100) { var %base.hp.tp $round($calc(160 * %current.battlestreak * 10),0) | set %hpadd %base.hp.tp }
  else { var %base.hp.tp $round($calc(140 * %current.battlestreak * 10),0) }


  if (%base.hp.tp > 20000) { var %base.hp.tp 20000 }

  writeini $char(%monster.name) basestats hp %hpadd
  writeini $char(%monster.name) basestats tp %base.hp.tp
  writeini $char(%monster.name) basestats str 16
  writeini $char(%monster.name) basestats def 24
  writeini $char(%monster.name) basestats int 28
  writeini $char(%monster.name) basestats spd 12

  $fulls(%monster.name) 

  if ((%battle.type = boss) && (%current.battlestreak >= 100)) { $levelsync(%monster.name, 110) }
  if ((%battle.type = boss) && (%current.battlestreak < 100)) {  $levelsync(%monster.name, $calc(%current.battlestreak + 10)) }
  ...
  writeini $char(%monster.name) weapons equipped Mythril_Sword
  writeini $char(%monster.name) weapons Mythril_Sword %current.battlestreak
  writeini $char(%monster.name) weapons MetalShield 1
  writeini $char(%monster.name) weapons equippedLeft MetalShield
  ...
  writeini $char(%monster.name) skills Resist-poison 55
  writeini $char(%monster.name) skills Resist-paralysis 100
  writeini $char(%monster.name) skills DualWield 1
  writeini $char(%monster.name) descriptions char Is the last warrior of the army of divine warriors to defend the crystal with his shield and sword.

  set %curbat $readini($txtfile(battle2.txt), Battle, List) |  %curbat = $addtok(%curbat,%monster.name,46) |  writeini $txtfile(battle2.txt) Battle List %curbat | write $txtfile(battle.txt) %monster.name
  $set_chr_name(%monster.name) 
  $display.message(4 $+ %real.name  $+ Is ready to help the heros by protecting the PowerCrystal, battle)
  $display.message(12 $+ %real.name  $+ $readini($char(%monster.name), descriptions, char), battle)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
generate_hq-gen-boss {
  set %current.battlestreak $readini(battlestats.dat, Battle, WinningStreak)
  if (%current.battlestreak <= 0) { set %current.battlestreak 100 }

  var %monster.level 1
  if (%battle.type = HQ-Gen) { set %number.of.monsters.needed 0 }
  set %bountyboss $readini(battlestats.dat, Bounty, BossName)
  if ($readini(battlestats.dat, HQ-Gen, Failed) == $null) { set %monster.level $calc(100 * 1) }
  if ($readini(battlestats.dat, HQ-Gen, Failed) != $null) { var %power.powercore $readini(battlestats.dat, HQ-Gen, Failed) | set %monster.level $calc($current.battlestreak * 100 + %power.powercore) }

  set %monster.name %bountyboss | set %monster.realname $readini($boss(%bountyboss), BaseStats, Name)
  ...
  writeini $char(%monster.name) info gender its
  writeini $char(%monster.name) info gender2 its
  ;writeini $char(%monster.name) info ai_type berserker

  if (%current.battlestreak < 100) { var %base.hp.tp $round($calc(30 * %current.battlestreak * 1000),0) }
  else { set %base.hp.tp $round($calc(50 * %current.battlestreak * 1000),0) }

  writeini $char(%monster.name) basestats hp %base.hp.tp
  writeini $char(%monster.name) basestats tp %base.hp.tp
  writeini $char(%monster.name) basestats str $readini($boss(%bountyboss), BaseStats, Str)
  writeini $char(%monster.name) basestats def $readini($boss(%bountyboss), BaseStats, def)
  writeini $char(%monster.name) basestats int $readini($boss(%bountyboss), BaseStats, int)
  writeini $char(%monster.name) basestats spd $readini($boss(%bountyboss), BaseStats, spd)

  ;levelsync

  var %str $readini($char(%monster.name),battle, str)
  var %def $readini($char(%monster.name), battle, def)
  var %int $readini($char(%monster.name), battle, int)
  var %spd $readini($char(%monster.name), battle, spd)

  inc %str $armor.stat(%monster.name, str)
  inc %def $armor.stat(%monster.name, def)
  inc %int $armor.stat(%monster.name, int)
  inc %spd $armor.stat(%monster.name, spd)

  if (%str <= 5) { var %str 5 | writeini $char(%monster.name) battle str $readini($boss(%bountyboss), BaseStats, Str) }
  if (%def <= 5) { var %def 5 | writeini $char(%monster.name) battle def $readini($boss(%bountyboss), BaseStats, Def) }
  if (%int <= 5) { inc %int 5 | writeini $char(%monster.name) battle int $readini($boss(%bountyboss), BaseStats, Int) }
  if (%spd <= 5) { inc %spd 5 | writeini $char(%monster.name) battle spd $readini($boss(%bountyboss), BaseStats, Spd) }

  var %level.difference $round($calc(%monster.level - $get.full.level(%monster.name)),0)
  var %difference.ratio $calc(%monster.level / $get.full.level(%monster.name))
  var %current.loop 0 
  ...
  writeini $char(%monster.name) battle Str %str
  writeini $char(%monster.name) battle Def %def
  writeini $char(%monster.name) battle Int %int
  writeini $char(%monster.name) battle Spd %spd

  writeini $char(%monster.name) info levelsync yes

  writeini $char(%monster.name) Battle hp %base.hp.tp
  writeini $char(%monster.name) Battle tp %base.hp.tp
  writeini $char(%monster.name) battle str $readini($char(%monster.name), BaseStats, str)
  writeini $char(%monster.name) battle def $readini($char(%monster.name), BaseStats, def)
  writeini $char(%monster.name) battle int $readini($char(%monster.name), BaseStats, int)
  writeini $char(%monster.name) battle spd $readini($char(%monster.name), BaseStats, spd)

  ;boost_monster

  $boost_monster_hp(%monster.name)
  $fulls(%monster.name) 
  $levelsync(%monster.name, %monster.level)

  set %curbat $readini($txtfile(battle2.txt), Battle, List) |  %curbat = $addtok(%curbat,%monster.name,46) |  writeini $txtfile(battle2.txt) Battle List %curbat | write $txtfile(battle.txt) %monster.name
  $set_chr_name(%monster.name) 
  $display.message($readini(translation.dat, battle, EnteredTheBattle),battle) 
  $display.message(12 $+ %monster.name  $+ $readini($char(%monster.name), descriptions, char), battle)
  ...
  if (%current.battlestreak < 100) {  writeini $txtfile(battle2.txt) battle alliednotes $rand(1,200) }
  if ((%current.battlestreak >= 100) && (%current.battlestreak < 500)) { writeini $txtfile(battle2.txt) battle alliednotes $rand(1,300) }
  if ((%current.battlestreak >= 500) && (%current.battlestreak < 1000)) { writeini $txtfile(battle2.txt) battle alliednotes $rand(1,400) }
  if ((%current.battlestreak >= 1000) && (%current.battlestreak < 2000)) { writeini $txtfile(battle2.txt) battle alliednotes $rand(1,600) }
  if ((%current.battlestreak >= 2000) && (%current.battlestreak < 3000)) { writeini $txtfile(battle2.txt) battle alliednotes $rand(1,900) }
  if (%current.battlestreak >= 3000) {  writeini $txtfile(battle2.txt) battle alliednotes $rand(1,1100) }
...
generate_hq-gen {
  ...
  set %monster.name HQ-Generator | set %monster.realname Allied Forces Energy Generator

  .copy -o $npc(%monster.name) $char(%monster.name)
  writeini $char(%monster.name) info flag npc
  writeini $char(%monster.name) Basestats name %monster.realname
  writeini $char(%monster.name) info password .8V%N)W1T;W5C:'1H:7,`1__.1134
  writeini $char(%monster.name) info gender its
  writeini $char(%monster.name) info gender2 its
  writeini $char(%monster.name) info ai_type defender
  writeini $char(%monster.name) info RageMode Ignore

  writeini $char(%monster.name) basestats str 4
  writeini $char(%monster.name) basestats def 18
  writeini $char(%monster.name) basestats int 10
  writeini $char(%monster.name) basestats spd 10
  ...
  if (%str <= 5) { var %str 5 | writeini $char(%monster.name) battle str $readini($npc(%monster.name), BaseStats, Str) }
  if (%def <= 5) { var %def 5 | writeini $char(%monster.name) battle def $readini($npc(%monster.name), BaseStats, Def) }
  if (%int <= 5) { inc %int 5 | writeini $char(%monster.name) battle int $readini($npc(%monster.name), BaseStats, Int) }
  if (%spd <= 5) { inc %spd 5 | writeini $char(%monster.name) battle spd $readini($npc(%monster.name), BaseStats, Spd) }

  var %level.difference $round($calc(%monster.level1 - $get.full.level(%monster.name)),0)
  var %difference.ratio $calc(%monster.level1 / $get.full.level(%monster.name))
  var %current.loop 0 
  ...
  writeini $char(%monster.name) descriptions char is an PowerFul energy Generator

  writeini $char(%monster.name) Battle hp 40000
  writeini $char(%monster.name) Battle tp 10000
  writeini $char(%monster.name) basestats hp 40000
  writeini $char(%monster.name) basestats tp 10000
  ...
  var %succ $readini(battlestats.dat, HQ-gen, Success) 
  if (%succ <= 1) { set %monster.hp 100 | $boost_monster_hp($1, $2, %monster.hp) | return }
  if (%succ > 1) { 
    var %extralife %succ
    goto boost 
  }
  ...
  :boost
  if (ignoreHP isin $readini($char(%monster.name), info, BattleStats)) { return }
  if ($readini($char(%monster.name), info, IgnoreHP) = true) { return }
  set %hp $readini($char(%monster.name), BaseStats, HP)
  ...
  if (%hq-gen = on) { var %hp.modifier 2.33 }

  ; Set the HP 

  var %hp $round($calc(%hp + (%hp * %hp.modifier) * %extralife),0)
  ...
  writeini $char(%monster.name) BaseStats HP $round(%hp,0)
  writeini $char(%monster.name) Battle HP $round(%hp,0)
  writeini $char(%monster.name) Battle BoostedHP yes
  var %boosted $readini($char(%monster.name), Battle, BoostedHP)  
  if (%boosted != yes) { goto boost }
  if (%boosted == yes) { goto fulls }
...
generate_pcrystal {
  ...
  set %monster.name powercrystal | set %monster.realname Allied Forces PowerCrystal
  ...
  writeini $char(%monster.name) descriptions char a Massivly Magical PowerCrystal that shimmers in lightblue aura.
...
generate_zombie_minion {
  ...
  if (%battle.type != dungeon) {
    ...
    if (%current.battlestreak > $return_levelCapSettingMonster(ZombieMinion)) { set %current.battlestreak $return_levelCapSettingMonster(ZombieMinion) } 
  }
  if (%battle.type = dungeon) { set %current.battlestreak $return_winningstreak }

  set %monster.name Zombie_Junior $+ $1 | set %monster.realname Zombie Junior $1
  ...
  writeini $char(%monster.name) info gender2 him
  writeini $char(%monster.name) info bosslevel %current.battlestreak
  writeini $char(%monster.name) monster type zombie
  ...
  writeini $char(%monster.name) techniques zombiebite $round($calc(%current.battlestreak / 3),0)

  writeini $char(%monster.name) weapons equipped zombiefists
  writeini $char(%monster.name) weapons zombiefists $round($calc(%current.battlestreak / 3),0)
  remini $char(%monster.name) weapons Fists

  writeini $char(%monster.name) skills Resist-stun 30
  writeini $char(%monster.name) skills Resist-charm 70
  writeini $char(%monstername) skills Cover 1

  writeini $char(%monster.name) descriptions char is a decayed corpse of a teenager.
...
generate_Zombieking {
  ...
  writeini $char(%monster.name) info gender his
  writeini $char(%monster.name) info gender2 him
  writeini $char(%monster.name) info Guardian Zombie_Junior.Zombie_Junior1.Zombie_Junior2.Zombie_Junior3.Zombie_Junior4
  writeini $char(%monster.name) info bosslevel %current.battlestreak
  writeini $char(%monster.name) monster type Zombie
  ...
  writeini $char(%monster.name) basestats str $rand(60,100)
  writeini $char(%monster.name) basestats def $rand(25,55)
  writeini $char(%monster.name) basestats int $rand(20,65)
  writeini $char(%monster.name) basestats spd $rand(65,165)

  writeini $char(%monster.name) techniques FastBlade %current.battlestreak
  writeini $char(%monster.name) techniques BurningBlade %current.battlestreak
  writeini $char(%monster.name) techniques FlatBlade %current.battlestreak
  writeini $char(%monster.name) techniques zombiebite %current.battlestreak

  writeini $char(%monster.name) weapons equipped LargeCutlass
  writeini $char(%monster.name) weapons LargeCutlass $round($calc(%current.battlestreak / 2),0)
  writeini $char(%monster.name) weapons Zombiefists $round($calc(%current.battlestreak / 2),0)
  ...
  writeini $char(%monster.name) descriptions char Is the zombie king called %kingname with his retinue of children who protect him.
  writeini $char(%monster.name) descriptions snatch tries to take a hostage!
  writeini $char(%monster.name) descriptions BossQuote Arrrr!  If you kill my children you will regret it badly.!!!
