...
on 3:TEXT:!monsterdrop*:#: { 
  remove drops.txt
  msg # i'm searching for 11 $2 in all Monsters & Bosses... Please Wait....
  set %dropitem $2
  set %itemlines $lines($dbfile(monsterdrops.db))
  if (%itemlines == 0) {
    msg # 4 14]7:::14[4Es sind keine op´s in # Zu Sehen14]7:::14[
  }
  if (%itemlines > 0 ) {
    :next 
    var %Monster $read  -l $+ %itemlines $dbfile(monsterdrops.db)
    var %Monster2 $replace(%Monster,=,.)
    var %out $gettok(%Monster2,1,46)
    if (%dropitem isin %Monster) {
      write -l $+ %itemlines drops.txt  14,1 ]3:9:3:14[4 $iif($isfile($mon(%out)) = $true, 7Monster: %out, 4Boss: %out) 14]3:9:3:14[
    }
    dec %itemlines
    if %itemlines == 0 goto done
    else goto next
    :done
    play #AiroRPG drops.txt
  }
}
...
on 3:TEXT:!using*:*: {  unset %real.name | unset %enemy | $set_chr_name($nick)
...
alias uses_item {
  ...
  if (%item.type = taming) {
    ...
    if ($readini($char($1), Taming_Monster, Name) == $null) { $display.message(Sorry you has not tame a monster to increase the stats, private) | halt }
    var %taming.stat $readini($dbfile(items.db), $2, Target)
    var %taming.amount $readini($dbfile(items.db), $2, Amount)
    var %current.taming.stat $readini($char($1), Taming_Monster, %taming.stat)
    var %new.tamingstat $calc(%current.taming.stat + %taming.amount)
    writeini $char($nick) Taming_Monster %taming.stat %new.tamingstat
    $display.message(3 You has increased the %taming.stat for %taming.amount of your $readini($char($1), Taming_Monster, Name) its %taming.stat is now $readini($char($1), Taming_Monster, %taming.stat), battle)
    $decrease_item($1, $2) | halt
  }

  if (%item.type = tamingClear)  {
    if ($readini($char($1), Taming_Monster, Name) == $null) { $display.message(Sorry you has not tame a monster to release it, private) | halt }
    var %monster $readini($char($1), Taming_Monster, Name)
    $display.message(3 You has now erased your data from the PoweredPokeball for the monster %monster . Now you has 1 EmptyPokeball, battle)
    remini $char($1) Taming_Monster
    $decrease_item($1, $2) | halt
    writeini $char($1) Item_Amount PoweredPokeball 0
    writeini $char($1) Item_Amount EmptyPokeball 1
  ...
  if (%item.type = infected) {
    var %amount $readini($dbfile(items.db), $2, Amount)
    var %desc $readini($dbfile(items.db), $2, Amount)
    set %randoms $rand(1, 100)
    if (%randoms <= 25) && (%randoms >= 10) { writeini $dbfile(items.db) $2 Desc Consumes a %item.type $2 and his HP was Refresched by %amount $+ % of the Max HP | $item.heal($1, $4, $2) }
    if (%randoms > 25) && (%randoms <= 45) { writeini $dbfile(items.db) $2 Desc Consumes a %item.type $2 that was not good %enemy gets damage from the %item.type $2 | $item.damage($1, $4, $2)) }
    if (%randoms >= 60) { 
      var %randstat $rand(1,100)
      if (%randstat < 50) { writeini $dbfile(items.db) $2 Desc Consumes a %item.type $2 and $1 transforms into a zombie for few times | /writeini $char($1) Status Zombie yes | $display.message($readini($dbfile(items.db), $2, desc),battle) }
      if (%randstat > 50) { writeini $dbfile(items.db) $2 Desc Consumes a %item.type $2 and $1 gets poisioned ugh... its not good | /writeini $char($1) Status Poison yes | $display.message($readini($dbfile(items.db), $2, desc),battle) }
    }
    if (%randoms < 10) || (%randoms == 59) || (%randoms == 50) || (%randoms == 49) { var %message 3Nothing happens by consuming a %item.type $2 it was %randoms  | $display.message(%message ,battle) }
  }
  ...
  if (%item.type = heal) {
    ...
    ; Decrease the action points
    $action.points($1, remove, 2)
    if (%hq-gen = on) && ($4 isin HQ-Generator) { return }
    $item.heal($1, $4, $2)
  }
  ...
  if (%item.type = tp) { 
    ...
    $display.message(03 $+ %real.name  $+ $readini($dbfile(items.db), $2, desc), battle)
    $item.tp($1, $4, $2)
    $decrease_item($1, $2) 
...
alias item.summon {
  ...
  if ($readini($dbfile(items.db), $2, desc) != $null) {  $set_chr_name($1)  | $display.message(03 $+ %real.name  $+ $readini($dbfile(items.db), $2, desc), battle) }
...
alias item.open.chest {
  ...
  if (%chest.item = $null) { $display.message(04ERROR: Chest item is null. Alert the bot admin to check the chest .lst file) | halt }
  ...
  if (%current.battlestreak >= 50) {
    ...
    if (%pcrystalrun = on) { var %mimic.chance -1000 }
  ...
  if (%chest.item = BlackOrb) { 
    ...
    if (%chest.item = $null) { echo -a 04,1ERROR Chest Item is NULL }
...
alias item.trust {
  ...
  if (%custom.summon.desc != $null) { $display.message(04 $+ %custom.summon.desc) }
  else { $display.message(04 $+ %real.name has been summoned to the battlefield to help $get_chr_name($1) $+ !,battle) }
...
alias item.tormentreward {
  ...
  if (%items.lines = $null) { $display.message(04ERROR: items_tormentrewad.lst is missing or empty!, private) | halt }
  ...
  if ($4 = 1) { 
    ...
    $display.message(03 $+ %real.name  $+ has obtained: %lifeshards.count lifeshards $+ $chr(44)  $+ %reusableparts.count resuable parts $+ $chr(44)  $+ %forgottensouls.count forgotten souls $+ $chr(44) and %arcanedust.count arcane dust  , global) 
  }
...
alias display_Statusdamage_item {
  ...
  if ($readini($dbfile(items.db), $4, amount) != 0) {
    if (%guard.message = $null) { $display.message(The attack did04 $bytes(%attack.damage,b) damage %style.rating, battle) }
...
alias item.ig {
  ...
  $display.message(03 $+ %enemy has regained %ig.amount Ignition Gauge!, battle)
  return 
}
...
alias item.shopreset {
  ...
  if (%showdiscountmessage = true) {
    if (%battleis != on) { $display.message(03 $+ %real.name $+  $readini($dbfile(items.db), $3, desc), battle) }
    if (%battleis = on) { $display.private.message(03 $+ %real.name $+  $readini($dbfile(items.db), $3, desc)) }
  }
...
alias item.food {
  ...
  if (%food.bonus <= 0) { 
    if ($2 != $1) { unset %food.bonus | $display.private.message(04This item cannot be used on other players. Only on monsters or yourself.) | halt }
  }
  ...
  while (%current.food.type <= $numtok(%food.type.list, 46)) {
    ...
    if (%food.type = stohlenorbs) {
      var %current.redorbs $readini($char($2), stuff, redorbs)
      var %stolenorbs $readini($char($2), stuff, StolenOrbs)
      var %reset.orbs $round($calc(%current.redorbs + %stolenorbs), 0)
      if (%stolenorbs != $null) {
        writeini $char($2) stuff redorbs %reset.orbs
        remini $char($2) stuff stolenorbs
        var %food.type Red Orbs
      }
    }
  ...
  $set_chr_name($1) 
  if (%battleis = on) { 
    if ($readini($char($2), info, flag) = $null)  { $display.private.message(03 $+ %real.name $+  $readini($dbfile(items.db), $3, desc)) }
    if ($readini($char($2), info, flag) != $null) { $display.message(03 $+ %real.name $+  $readini($dbfile(items.db), $3, desc), global) }
  }
  if (%battleis = off) { $display.message(03 $+ %real.name $+  $readini($dbfile(items.db), $3, desc), global) }
...
on 3:TEXT:!carry*:*: {  
  if ($3 = $null) { $display.private.message(04Error: !carry <accessory/armor> <what to wear>, private) | halt }
  if ($2 = accessory) { 
...
on 3:TEXT:!remove*:*: {  
  if ($3 = $null) { $display.private.message(04Error: !remove <accessory/armor> <what to remove>, private) | halt }
  if ($2 = accessory) { 
...
alias portal.item.onemonster {
  ...
  if (%bossquote != $null) {   $display.message(02 $+ %real.name looks at the heroes and says " $+ $readini($char(%monster.to.spawn), descriptions, BossQuote) $+ ", battle) }
...
alias item.torment {
  ...
  set %torment.wave 1
  if (%torment.level = $null) { echo -a 04ERROR: TORMENT LEVEL NULL | set %torment.level 1 }
