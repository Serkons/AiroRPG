...
on 1:TEXT:!create char*:*: { $checkscript($2-) | $create.new.character($nick) }

alias create.new.character { 
  if ($isfile($char($1)) = $true) && ($readini($char($nick), Info, Tutorial) != on) { $display.private.message($readini(translation.dat, system, PlayerExists)) | halt }
  ...
  if ($1 = Zombie_Junior) { $display.private.message($readini(translation.dat, system, NameReserved)) | halt }
  if ($1 = pirate_firstmatey) { $display.private.message($readini(translation.dat, system, NameReserved)) | halt }
  if (Zombie isin $1) { $display.private.message($readini(translation.dat, system, NameReserved)) | halt }
  ...
  var %tut $readini($char($1), Info, Tutorial)
  if (%tut == on) {
    ...
    writeini $char($1) Info Tutorial on
  }
  if (%tut == $null) {
    ; Create the new character now
    /copy $char(new_chr) $char($1)
  ...
  ; Generate a password
  set %password AiroRPG $+ $rand(1,100) $+ $rand(a,z)
...
ON 2:TEXT:!newpassword *:?:{ $checkscript($2-) | $password($nick) 
...
; Check bot version
on 2:TEXT:!version:*: {
  $display.message(12Airo RPG Version04 $battle.version,private) 
}
...
on 3:TEXT:!mycdesc *:*:{ $checkscript($2-)  | writeini $char($nick) Descriptions Char $2- | $okdesc($nick , Character) }
...
on 3:TEXT:!myskill desc *:*:{ $checkscript($3-)  | $set_chr_name($nick) 
...
on 3:TEXT:!clear mydesc *:*:{ $checkscript($3-)  | $set_chr_name($nick) 
...
on 3:TEXT:!set mydesc *:*:{ $checkscript($2-)  
  if ($3 = $null) { $display.private.message(04Invalid description) | halt }
  writeini $char($nick) Descriptions Char $3- | $okdesc($nick , Character) 
}

on 3:TEXT:!setmygender*:*: { $checkscript($2-)
...
on 3:TEXT:!Mylevel*:#: { $checkscript($2-)
  if ($1 = !myleveladjust) { halt }
  if (($1 = !mylevel) && ($2 = sync)) {
...
on 50:TEXT:!mylevel sync*:#: { $checkscript($2-)
  if ($1 = !myleveladjust) { halt }
  if ($3 = $null) { $display.private.message(04Error: Need to enter a number of level to sync to) | halt }
  if ($3 <= 0) { $display.private.message(04Error: Cannot sync to level 0 or lower) | halt }
  $levelsync($nick, $3)
...
on *:TEXT:!login:*: {
  if ($readini(system.dat, system, botType) != TWITCH) {
    ...
    if ($isfile($char($nick)) = $true) && ($istok(%bot.owners,%nick, 46) != $true) {
      .auser 3 $nick
      var %nick $nick
      var %passwdadada $readini($char($nick), Info, Password)
      var %dercyptpassword $decode(%passwdadada)
      $set_chr_name($nick) 
      var %bot.owners $readini(system.dat, botinfo, bot.owner)
      if ($istok(%bot.owners,$nick,46) = $true) {
        var %bot.owner $gettok(%bot.owners, 1, 46)
      ...
      mode %battlechan +v $nick | $id_login($nick) | unset %passhurt | $writehost($nick, $site) | $system.intromessage($nick) | /close -m* | $set_chr_name($nick) | $display.message(10 $+ %real.name %custom.title  $+ $readini($char($nick), Descriptions, Char), global)
      $idcheck($nick, %dercyptpassword) | mode %battlechan +v %nick  |  unset %passhurt | $writehost($nick, $site) |  $system.intromessage($nick) | /close -m* 
      if ($readini($char($nick), info, CustomTitle) != $null) { var %custom.title " $+ $readini($char($nick), info, CustomTitle) $+ " }
      if (($readini(system.dat, system, botType) = IRC) || ($readini(system.dat, system, botType) = TWITCH)) { $set_chr_name(%nick ) | $display.message(10 $+ %real.name %custom.title  $+  $readini($char($nick), Descriptions, Char), global) }
    }
  }
...
on 3:TEXT:!mystats*:*: { unset %all_status 
  if ($2 = $null) { 
    ...
    $display.private.message.delay.custom([04HP12 $readini($char($nick), Battle, HP) $+ 1/ $+ 12 $+ $readini($char($nick), BaseStats, HP) $+ ] [04TP12 $readini($char($nick), Battle, TP) $+ 1/ $+ 12 $+ $readini($char($nick), BaseStats, TP) $+ ] [04Ignition Gauge12 $readini($char($nick), Battle, IgnitionGauge) $+ 1/ $+ 12 $+ $readini($char($nick), BaseStats, IgnitionGauge) $+ ] [04Level12 $get.level($nick) $+ ] [04Armor Level12 $ilevel($nick) $+ ] [04Armor Protection12 $armor.protection($nick) $+ $chr(37)  $+ ] [04Status12 %all_status $+ ] [04Royal Guard Meter12 %blocked.meter $+ ] [04Capacity Points12 $readini($char($nick), stuff, CapacityPoints)  $+ 1/12 $+ 10000 $+ ] [04Enhancement Points12 $readini($char($nick), stuff, EnhancementPoints) $+ ] [04Login Points12 $readini($char($nick), stuff, LoginPoints) $+ ]  ,2)
    ...
    if ($readini($char($nick), Taming_Monster, Name) != $null) {
      var %name $readini($char($nick), Taming_Monster, Name)
      var %tHP $readini($char($nick), Taming_Monster, HP)
      var %ttp $readini($char($nick), Taming_Monster, TP)
      var %tstr $readini($char($nick), Taming_Monster, Str)
      var %tdef $readini($char($nick), Taming_Monster, Def)
      var %tint $readini($char($nick), Taming_Monster, Int)
      var %tspd $readini($char($nick), Taming_Monster, Spd)
      var %tig $readini($char($nick), Taming_Monster, IgnitionGauge)
      var %tLevel $readini($char($nick), Taming_Monster, Level)
      $display.private.message.delay.custom([4Pokemon Name: 12 %name $+ ] [4Level: 12 %tlevel $+ ] [4HP:12 %tHP 4 TP:12 %ttp  4 STR:12 %tstr 4 DEF:12 %tdef 4 SPD:12 %tspd 4 IG:12 %tig 4 $+ ],5)
    }
    ;if ($readini($char($nick), Taming_Monster, NaturalArmor-Name) == $null) && ($readini($char($nick), Taming_Monster, Name) == $null) { var %naturalarmor none | $display.private.message.delay.custom([4Natural Armor: 12 %naturalarmor $+ ],6) }
    if ($readini($char($nick), Taming_Monster, NaturalArmor-Name) != $null) {

      var %armorname $readini($char($nick), Taming_Monster, NaturalArmor-Name)
      var %armormax $readini($char($nick), Taming_Monster, NaturalArmor-Max)
      var %armorcur $readini($char($nick), Taming_Monster, NaturalArmor-Current)

      $display.private.message.delay.custom([4Natural Armor: 12 %armorname $+ ] [4Max: 12 %armormax $+ ] [4Current: 12 %armorcur $+ ],7)
    ...
    if ($readini($char($2), Taming_Monster, Name) != $null) {
      var %name $readini($char($2), Taming_Monster, Name)
      var %tHP $readini($char($2), Taming_Monster, HP)
      var %ttp $readini($char($2), Taming_Monster, TP)
      var %tstr $readini($char($2), Taming_Monster, Str)
      var %tdef $readini($char($2), Taming_Monster, Def)
      var %tint $readini($char($2), Taming_Monster, Int)
      var %tspd $readini($char($2), Taming_Monster, Spd)
      var %tig $readini($char($2), Taming_Monster, IgnitionGauge)
      var %tLevel $readini($char($2), Taming_Monster, Level)
      $display.private.message.delay.custom([4Pokemon Name: 12 %name $+ ] [4Level: 12 %tlevel $+ ] [4HP:12  4 TP:12 %tHP  4 STR:12 %tstr 4 DEF:12 %tdef 4 SPD:12 %tspd 4 IG:12 %tig 4 $+ ])
    }
    if ($readini($char($2), Taming_Monster, NaturalArmor-Name) == $null) { var %naturalarmor none | $display.private.message.delay.custom([4Natural Armor: 12 %naturalarmor $+ ] }
    if ($readini($char($2), Taming_Monster, NaturalArmor-Name) != $null) {

      var %armorname $readini($char($2), Taming_Monster, NaturalArmor-Name
      var %armormax $readini($char($2), Taming_Monster, NaturalArmor-Max
      var %armorcur $readini($char($2), Taming_Monster, NaturalArmor-Current

      $display.private.message.delay.custom([4Natural Armor: 12 %armorname $+ ] [4Max: 12 %armormax $+ ] [4Current: 12 %armorcur $+ ])
...
alias crystalpower {
  var %crystal.percent $readini(battlestats.dat, PowerCrystal, Power)

  if (%crystal.percent = 0) { return $readini(translation.dat, crystalstatus, low) }
  if (%crystal.percent > 0) && (%crystal.percent <= 5) { return $readini(translation.dat, crystalstatus, low) }  
  if (%crystal.percent > 6) && (%crystal.percent <= 10) { return $readini(translation.dat, crystalstatus, lowen) }
  if (%crystal.percent > 10) && (%crystal.percent <= 20) { return $readini(translation.dat, crystalstatus, charging) }
  if (%crystal.percent > 20) && (%crystal.percent <= 30) { return $readini(translation.dat, crystalstatus, low_charging) }
  if (%crystal.percent > 30) && (%crystal.percent <= 40) { return $readini(translation.dat, crystalstatus, loading) }
  if (%crystal.percent > 40) && (%crystal.percent <= 50) { return $readini(translation.dat, crystalstatus, moren) }
  if (%crystal.percent > 50) && (%crystal.percent <= 60) { return $readini(translation.dat, crystalstatus, medium_state) }
  if (%crystal.percent > 60) && (%crystal.percent <= 70) { return $readini(translation.dat, crystalstatus, medium_state_loaded) }
  if (%crystal.percent > 70) && (%crystal.percent <= 80) { return $readini(translation.dat, crystalstatus, Most_en) }
  if (%crystal.percent > 80) && (%crystal.percent <= 95) { return $readini(translation.dat, crystalstatus, GoodEn) }
  if (%crystal.percent > 95) && (%crystal.percent <= 100) { return $readini(translation.dat, crystalstatus, Crit_re) }
  if (%crystal.percent >= 100) { return $readini(translation.dat, crystalstatus, Critical) }                      
}

on 3:TEXT:!the monster strength:*: {
...
on 3:TEXT:!watch*:#: { unset %all_status 
...
on 3:TEXT:!Theweapons*:#: {  unset %*.wpn.list | unset %weapon.list
...
on 3:TEXT:!Theshields*:#: {  unset %*.shld.list | unset %shield.list
...
on 3:TEXT:!mystyle*:*: {  unset %*.style.list | unset %style.list
  if ($2 = $null) { 
...
on 3:TEXT:!Mytechs*:#: {  unset %techs.list | unset %ignition.tech.list
...
on 3:TEXT:!tameitems*:#:{ 
  if ($3 != $null) { $checkchar($3) | $tamingitems.list($3) | $set_chr_name($3) | $readtamingitems($3, channel) }
  else {  $tamingitems.list($nick) | $set_chr_name($nick) | $readtamingitems($nick, channel) }
}
on 3:TEXT:!tameitems*:?:{ 
  if ($3 != $null) { $checkchar($3) | $tamingitems.list($3) | $set_chr_name($3) | $readtamingitems($3, channel) }
  else {  $tamingitems.list($nick) | $set_chr_name($nick) | $readtamingitems($nick, channel) }
...
on 3:TEXT:!mystatus:#: { $player.status($nick) | $display.message($readini(translation.dat, system, ViewStatus),private) | unset %all_status } 
on 3:TEXT:!mystatus:?: { $player.status($nick) | $display.private.message($readini(translation.dat, system, ViewStatus)) | unset %all_status }
...
;;;; AUGMENT CMNDS
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
on 3:TEXT:!forging*:#:{ $reforge.weapon($nick, $2) }
...
; ===================================
on 3:TEXT:!go dragon hunt*:*:{ 
  if (%battleis = on) { $display.message($readini(translation.dat, errors, Can'tDoThisInBattle), private) | halt }
  $character.dragonhunt($nick) 
} 
on 3:TEXT:!the dragon list*:*:{ 
  if (%battleis = on) { $display.message($readini(translation.dat, errors, Can'tDoThisInBattle), private) | halt }
