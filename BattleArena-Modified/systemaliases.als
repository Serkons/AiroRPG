...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
battle.version {  
  if ($readini(version.ver, versions, Bot) = $null) { echo -a 04ERROR: version.ver is either missing or corrupted! | return 4.0 }
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
system.dat.version { 
  if ($readini(version.ver, versions, systemdat) = $null) { echo -a 04ERROR: version.ver is either missing or corrupted! | return 0 }
...
system_defaults_check {
  ...
  if (%last.system.dat.version != $system.dat.version) { 
    ...
    if ($readini(system.dat, system, EnableZombies) = $null) { writeini system.dat system EnableZombie true }
    ...
    if ($readini(system.dat, system, ShowPlayerLevelUp) = $null) { writeini system.dat system ShowPlayerLevelUp false }
    ...
    if ($readini(system.dat, PlayerLevelCaps, Zombies) = $null) { writeini system.dat PlayerLevelCaps Zombies 85 }
    ...
    if ($readini(system.dat, MonsterLevelCaps, ZombieMinion) = $null) { writeini system.dat MonsterLevelCaps PirateMinion 80 }
    if ($readini(system.dat, MonsterLevelCaps, PirateFirstMatey) = $null) { writeini system.dat MonsterLevelCaps PirateFirstMatey 73 }
    if ($readini(system.dat, MonsterLevelCaps, ZombieKing) = $null) { writeini system.dat MonsterLevelCaps ZombieKing 83 }
...
system.start.newbattle {
  ...
  if ($1 = rakna) { $item.dungeon(Bark Spider, RaknaPortal, true) | halt }
  ...
  if ((%special.flag = $null) && ($current.battlestreak >= 1)) {
    var %random.chance $rand(1,255)
    if (%random.chance >= 250) { $item.dungeon(Pirates, PirateShipEvent, true) | halt }  
  }

  if ((%special.flag = $null) && ($current.battlestreak >= 1)) {
    set %random.chance1 $rand(1,255)
    if (%random.chance1 >= 240) {
      var %dfolder  $mircdir $+ dungeons\

      set %dname $read(dungnames.txt)
      var %rooms $rand(6, 20)
      var %froom $calc(%rooms - 1)
      var %topmon $rand(1,$findfile($mon_path , *.char, 0))
      var %topboss $rand(1,$findfile($boss_path , *.char, 0))
      writeini %dfolder $+ CreatedDungeon $+ .dungeon Info Name %dname
      writeini %dfolder $+ CreatedDungeon $+ .dungeon Info Rooms %rooms
      writeini %dfolder $+ CreatedDungeon $+ .dungeon Info PlayersNeeded 1
      writeini %dfolder $+ CreatedDungeon $+ .dungeon Info Level $current.battlestreak
      writeini %dfolder $+ CreatedDungeon $+ .dungeon Info Bosslevel $calc($current.battlestreak + 10) 
      writeini %dfolder $+ CreatedDungeon $+ .dungeon Info RedOrbAmount $rand(500,1000)
      writeini %dfolder $+ CreatedDungeon $+ .dungeon Info FinalRoom %froom
      writeini %dfolder $+ CreatedDungeon $+ .dungeon Info StartBattleDesc The Spys from the Allied forces have found a Dungeon called %dname . with 4 %rooms 2rooms you can clear it to get the reward of it.
      writeini %dfolder $+ CreatedDungeon $+ .dungeon Info ClearDungeonDesc The Dungeon was cleared by the heroes good job [Clear time: %total.battle.duration $+ ]
      writeini %dfolder $+ CreatedDungeon $+ .dungeon Info DungeonFail The Heroes cant clear this dungeon. try it next time
      writeini %dfolder $+ CreatedDungeon $+ .dungeon 0 Desc 2[Room: 7 $+ Lobby $+ 2] 2 This is the first room of the dungeon you can use skills and items if you done you can type !rest or !next to go to the next room.
      writeini %dfolder $+ CreatedDungeon $+ .dungeon 0 Battlefield %dname


      writeini %dfolder $+ CreatedDungeon $+ .dungeon %froom Desc 2[Room: 8 $+ BossRoom $+ 2] The Heroes goes to the end room there are the boss %bossname, You must defeat him to clear the dungeon
      writeini %dfolder $+ CreatedDungeon $+ .dungeon %froom bossroom true
      writeini %dfolder $+ CreatedDungeon $+ .dungeon %froom battlefield %dname
      var %bossnum $rand(1,$findfile( $boss_path , *.char, %topboss)), .char)
      var %bossfile $findfile($boss_path , *.char, %topboss)
      var %bossname $remove(%bossfile,  $mircdir $+ bosses\, .char)
      /timerbossroom1 1 5 writeini %dfolder $+ CreatedDungeon $+ .dungeon %froom Monsters %bossname
      /timerbossroom2 1 5 writeini %dfolder $+ CreatedDungeon $+ .dungeon %froom Desc 2[Room: 8 $+ BossRoom $+ 2] The Heroes goes to the end room there are the boss  $+ %bossname $+  , You must defeat him to clear the dungeon
      $mondung
      ;writeini %dfolder $+ CreatedDungeon $+ .dungeon MonsterList 1 $remove($findfile($mon_path, *.char, $rand(1,$findfile($mon_path, *.char, 0))),  $mircdir $+ monsters\, .char) $+ . $+ $remove($findfile($mon_path, *.char, $rand(1,$findfile($mon_path, *.char, 0))),  $mircdir $+ monsters\, .char) $+ . $+ $remove($findfile($mon_path, *.char, $rand(1,$findfile($mon_path, *.char, 0))),  $mircdir $+ monsters\, .char)
      ;writeini %dfolder $+ CreatedDungeon $+ .dungeon MonsterList 2 $remove($findfile($mon_path, *.char, $rand(1,$findfile($mon_path, *.char, 0))), $mircdir $+ monsters\, .char) $+ . $+ $remove($findfile($mon_path, *.char, $rand(1,$findfile($mon_path, *.char, 0))), $mircdir $+ monsters\, .char) $+ . $+ $remove($findfile($mon_path, *.char, $rand(1,$findfile($mon_path, *.char, 0))), $mircdir $+ monsters\, .char)
      ;writeini %dfolder $+ CreatedDungeon $+ .dungeon MonsterList 3 $remove($findfile($mon_path, *.char, $rand(1,$findfile($mon_path, *.char, 0))), $mircdir $+ monsters\, .char) $+ . $+ $remove($findfile($mon_path, *.char, $rand(1,$findfile($mon_path, *.char, 0))), $mircdir $+ monsters\, .char) $+ . $+ $remove($findfile($mon_path, *.char, $rand(1,$findfile($mon_path, *.char, 0))), $mircdir $+ monsters\, .char)
      ;writeini %dfolder $+ CreatedDungeon $+ .dungeon MonsterList 4 $remove($findfile($mon_path, *.char, $rand(1,$findfile($mon_path, *.char, 0))), $mircdir $+ monsters\, .char) $+ . $+ $remove($findfile($mon_path, *.char, $rand(1,$findfile($mon_path, *.char, 0))), $mircdir $+ monsters\, .char) $+ . $+ $remove($findfile($mon_path, *.char, $rand(1,$findfile($mon_path, *.char, 0))), $mircdir $+ monsters\, .char)
      ...
      while (%x <= %froom) {
        ...
        set %bossroom1 $readini(%dfolder $+ CreatedDungeon $+ .dungeon, %froom, bossroom)
        echo -a Bosschance: %rand.chance
        ;if ($readini(%dfolder $+ CreatedDungeon $+ .dungeon, %x, ChestRoom) == true) { echo -a ChestRaum erstellt: Raumnummer %chestroomnum }
        ;if ($readini(%dfolder $+ CreatedDungeon $+ .dungeon, %x, Restoreroom) == true) { echo -a RestoreRaum erstellt: Raumnummer %restraumnum }
        if ($readini(%dfolder $+ CreatedDungeon $+ .dungeon, %x, ChestRoom) == $null) && ($readini(%dfolder $+ CreatedDungeon $+ .dungeon, %x, RestoreRoom) == $null) { echo -a Monsterraum Erstellt: Raumnummer %x | writeini %dfolder $+ CreatedDungeon $+ .dungeon %x Monsters $readini($dungeonfile(CreatedDungeon), MonsterList, $rand(1,11)) }
        inc %x
      }
      remini %dfolder $+ CreatedDungeon $+ .dungeon %froom ChestRoom
      remini %dfolder $+ CreatedDungeon $+ .dungeon %froom RestoreRoom
    $item.dungeon(Warning, CreateDungeon, true) | $dungeon.content | halt }
  }
...
checkchar2 {
  var %check $readini($temp($1), Stuff, ShopLevel)
  if (%check = $null) { $display.message($readini(translation.dat, errors, NotInDataBank), private) | halt }
  else { return }
}
...
; Paths
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
temp { return " $+ $mircdir $+ %tempfolder $+ $1 $+ .char" }
...
display.message {
  ...
  if ($readini(system.dat, system, botType) = TWITCH) {
    ...
    /.timerThrottleDisplayMessage $+ $2 $+ $rand(1,100) $+ $rand(a,z) $+ $rand(1,1000) -d 1 %twitch.delay /msg %battlechan %message.to.display 
  }
...
status_message_check { 
  if (%all_status = $null) { %all_status = 04 $+ $1- | return }
  else { %all_status = 04 $+ %all_status $+ $chr(0160) $+ 03 $+ $chr(124) $+ 04 $+ $chr(0160) $+ $1- | return }
}
...
skills_message_check { 
  if (%all_skills = $null) { %all_skills = 04 $+ $1- | return }
  else { %all_skills = 04 $+ %all_skills $+ $chr(0160) $+ 03 $+ $chr(124) $+ 04 $+ $chr(0160) $+ $1- | return }
}
...
idcheck { 
  ...
  if ($readini($char($1), info, banned) = yes) {  $display.private.message(04This character has been banned and cannot be used to log in.) | halt }
...
build_battlehp_list {
  ...
  while (%battletxt.current.line <= %battletxt.lines) { 
    ...
    if ($readini($char(%who.battle), info, flag) = monster) { inc %battletxt.current.line }
      ...
      var %hp.to.add  03 $+ $chr(91) $+  $+ %who.battle $+ :  %hstats $+ 03 $+ $chr(93) 
...
build_battlestyle_list {
  ...
  while (%current.person <= %number.of.people) {
    ...
    if ($readini($char(%person.name), info, flag) != monster) {
      ...
      var %style.to.add  03 $+ %person.name $+ :  %style.rating
  ...
  while (%value <= %items.lines) {
    ...
    if ($readini($dbfile(weapons.db), %item.name, type) != $null) {
      if ((%item_amount != $null) && (%item_amount >= 1)) { 
        if ($numtok(%mech.weapon.list,46) <= 20) { %mech.weapon.list = $addtok(%mech.weapon.list, 03 $+ %item.name, 46) }
        else { %mech.weapon.list2 = $addtok(%mech.weapon.list2, 03 $+ %item.name $+ $chr(040) $+ %item_amount $+ $chr(041), 46) }
      }
    }
  ...
  while (%weapon.list.place <= %total.weapon.lists) {
    ...
    while (%value <= %number.of.weapons) {
      ...
      if ((%weapon_level != $null) && (%weapon_level >= 1)) { 
        ...
        if ($readini($dbfile(weapons.db), %weapon.name, Legendary) = true) { var %weapon.color 07 }

        var %weapon_to_add  $+ %weapon.color $+ %weapon.name $+ 03 $+ $chr(040) $+ %weapon_level $+ $chr(041) $+ 
  ...
  while (%shield.list.place <= %total.shield.lists) {
    ...
    while (%value <= %number.of.shields) {
      ...
      if ((%shield_level != $null) && (%shield_level >= 1)) { 
        ...
        var %shield.name $equipment.color(%shield.name) $+ %shield.name $+ 03
...
skills.list2 {
  $passive.skills.list2($1)
  $active.skills.list2($1)
  set %resists.skills.list $resists.skills.list2($1)
  set %killer.skills.list $killer.skills.list2($1)
  unset %total.skills | unset %skill.name | unset %skill_level | unset %replacechar
  return
}
...
passive.skills.list2 { 
  ...
  while (%value <= %skills.lines) {
    set %skill.name $read -l $+ %value $lstfile(skills_passive.lst)
    set %skill_level $readini($temp($1), skills, %skill.name)
...
gems.list {
  ...
  while (%value <= %items.lines) {
    ...
    if ((%item_amount != $null) && (%item_amount >= 1)) { 
    %gems.items.list = $addtok(%gems.items.list, 07 $+ %item.name $+ $chr(040) $+ %item_amount $+ $chr(041), 46) }
...
items.list {
  ...
  while (%value <= %items.lines) {
    ...
    if ((%item_amount != $null) && (%item_amount >= 1)) { 
      if ($numtok(%items.list,46) <= 20) { %items.list = $addtok(%items.list, 03 $+ %item.name $+ $chr(040) $+ %item_amount $+ $chr(041), 46) }
      else { %items.list2 = $addtok(%items.list2, 03 $+ %item.name $+ $chr(040) $+ %item_amount $+ $chr(041), 46) }
    }
    unset %item.name | unset %item_amount
    inc %value 1 
  }
  ...
  while (%value <= %items.lines) {
    ...
    if ((%item_amount != $null) && (%item_amount >= 1)) { 
      if ($numtok(%items.list,46) <= 20) { %items.list = $addtok(%items.list, 04 $+ %item.name $+ $chr(040) $+ %item_amount $+ $chr(041), 46) }
      else { %items.list2 = $addtok(%items.list2, 04 $+ %item.name $+ $chr(040) $+ %item_amount $+ $chr(041), 46) }
    }
    unset %item.name | unset %item_amount
    inc %value 1 
  }
  ...
  while (%value <= %items.lines) {
    ...
    if ((%item_amount != $null) && (%item_amount >= 1)) { 
    %reset.items.list = $addtok(%reset.items.list, 02 $+ %item.name $+ $chr(040) $+ %item_amount $+ $chr(041), 46) }
...
tamingitems.list {
  ...
  ; CHECKING MISC ITEMS
  var %value 1 | unset %taming.items.* | var %items.lines $lines($lstfile(items_taming.lst))

  while (%value <= %items.lines) {
    set %item.name $read -l $+ %value $lstfile(items_taming.lst)
    set %item_amount $readini($char($1), item_amount, %item.name)
    if (%item_amount <= 0) { remini $char($1) item_amount %item.name }
    ...
    if ((%item_amount != $null) && (%item_amount >= 1)) { 
      if ($numtok(%taming.items.list,46) <= 15) { %taming.items.list = $addtok(%taming.items.list, %item.color $+ %item.name $+ 05 $+ $chr(040) $+ %item_amount $+ $chr(041), 46) }
      else { 
        if ($numtok(%taming.items.list2,46) <= 15) { %taming.items.list2 = $addtok(%taming.items.list2, %item.color $+ %item.name $+ 05 $+ $chr(040) $+ %item_amount $+ $chr(041), 46) }
        else { 
          if ($numtok(%taming.items.list3,46) <= 15) { %taming.items.list3 = $addtok(%taming.items.list3, %item.color $+ %item.name $+ 05 $+ $chr(040) $+ %item_amount $+ $chr(041), 46) }
          else { 
            if ($numtok(%taming.items.list4,46) <= 15) { %taming.items.list4 = $addtok(%misc.items.list4, %item.color $+ %item.name $+ 05 $+ $chr(040) $+ %item_amount $+ $chr(041), 46) }
            else { 
              if ($numtok(%taming.items.list5,46) <= 15) { %taming.items.list5 = $addtok(%taming.items.list5, %item.color $+ %item.name $+ 05 $+ $chr(040) $+ %item_amount $+ $chr(041), 46) }
              else { %taming.items.list6 = $addtok(%taming.items.list6, %item.color $+ %item.name $+ 05 $+ $chr(040) $+ %item_amount $+ $chr(041), 46) }
            } 
          }
        }
      }
  ...
  %taming.items.list = $replace(%taming.items.list, $chr(046), %replacechar)
  %taming.items.list2 = $replace(%taming.items.list2, $chr(046), %replacechar)
  %taming.items.list3 = $replace(%taming.items.list3, $chr(046), %replacechar)
  %taming.items.list4 = $replace(%taming.items.list4, $chr(046), %replacechar)
  %taming.items.list5 = $replace(%taming.items.list5, $chr(046), %replacechar)
  %taming.items.list6 = $replace(%taming.items.list6, $chr(046), %replacechar)
}
...
items.mechcore {
  ...
  while (%value <= %items.lines) {
    ...
    if ($readini($dbfile(items.db), %item.name, type) = mechCore) {
      if ((%item_amount != $null) && (%item_amount >= 1)) { 
        if ($numtok(%mech.items.list,46) <= 20) { %mech.items.list = $addtok(%mech.items.list, 03 $+ %item.name $+ $chr(040) $+ %item_amount $+ $chr(041), 46) }
        else { %mech.items.list2 = $addtok(%mech.items.list2, 03 $+ %item.name $+ $chr(040) $+ %item_amount $+ $chr(041), 46) }
      }
    }
    unset %item.name | unset %item_amount
    inc %value 1 
  ...
  while (%current.ini.item.num <= %number.of.ini.items) { 
    ...
    if (%current.ini.value = 0) { remini $char($1) item_amount %current.ini.item | dec %current.ini.item.num 1 }
      ...
      if (%item.type != $null) { 
        ...
        if (%armor.level > $get.level($1)) { var %armor.name 04 $+ %current.ini.item $+ 03 }
...
accessories.list {
  ...
  while (%value <= %items.lines) {
    ...
    if ((%item_amount != $null) && (%item_amount >= 1)) { 
      ...
      var %item.name $equipment.color(%item.name) $+  $+ %item.name $+ 03
...
runes.list {
  ...
  while (%value <= %items.lines) {
    ...
    if ((%item_amount != $null) && (%item_amount >= 1)) { 
    %runes.list = $addtok(%runes.list, 07 $+ %item.name $+ $chr(040) $+ %item_amount $+ $chr(041), 46) }
...
fulls {  
  ...
  if ($readini($char($1), info, flag) = monster) || ($readini($char($1), Info, flag) = npc) {
    if ((%players.must.die.mode = false) || (%players.must.die.mode = $null)) && (Armored isin $1)  {
      ;var %real.name2 $remove($char($1),  $mircdir $+ characters\,.char, ")
      ;var %real.name $replace(%real.name2, _, $chr(32))
      writeini $char($1) Battle Hp $readini($char($1), BaseStats, HP)
      writeini $char($1) Battle Tp $readini($char($1), BaseStats, TP)
      writeini $char($1) BaseStats Name %real.name
      var %armhp $readini($char($1), BaseStats, hp)
      var %armvalue2 $round($calc(%armhp * 2), 0)
      var %armvalue $rand(1, %armvalue2)
      var %armnumber $lines(Armornames.txt)
      var %armorlines $rand(1, %armnumber)
      var %armorname $read(Armornames.txt, %armorlines)
      writeini $char($1) NaturalArmor Name %armorname
      writeini $char($1) NaturalArmor Max %armvalue
      writeini $char($1) NaturalArmor Current %armvalue
    }
    if ((%players.must.die.mode = false) || (%players.must.die.mode = $null)) && (Generator isin $1)  {
      ;var %real.name2 $remove($char($1), $mircdir $+ characters\,.char, ")
      ;var %real.name $replace(%real.name2, _, $chr(32))
      writeini $char($1) Battle Hp $readini($char($1), BaseStats, HP)
      writeini $char($1) Battle Tp $readini($char($1), BaseStats, TP)
      ;writeini $char($1) BaseStats Name %real.name
      var %armhp $readini($char($1), BaseStats, hp)
      var %armvalue2 $round($calc(%armhp * 3), 0)
      var %armvalue $rand(1, %armvalue2)
      var %armnumber $lines(Armornames.txt)
      var %armorlines $rand(1, %armnumber)
      var %armorname $read(Armornames.txt, %armorlines)
    ...
    if ((%players.must.die.mode = false) || (%players.must.die.mode = $null)) && (SuperElite isin $1)  {
      ;var %real.name2 $remove($char($1), $mircdir $+ characters\, .char, ")
      ;var %real.name $replace(%real.name2, _, $chr(32)) 
      writeini $char($1) Battle Hp $readini($char($1), BaseStats, HP)
      writeini $char($1) Battle Tp $readini($char($1), BaseStats, TP)
      ...
      writeini $char($1) NaturalArmor Max %armvalue
      writeini $char($1) NaturalArmor Current %armvalue
      writeini $char($1) Skills RepairNaturalArmor 1
      writeini $char($1) Descriptions RepairNaturalArmor Uses same flint stones and a repairhammer to repair his %armorname
      writeini $char($1) monster SuperElite true
    }
    if ((%players.must.die.mode = false) || (%players.must.die.mode = $null)) && (Armored !isin $1) || (SuperElite !isin $1)  {
      writeini $char($1) Battle Hp $readini($char($1), BaseStats, HP)
      writeini $char($1) Battle Tp $readini($char($1), BaseStats, TP)
    }
    if (%players.must.die.mode = true)  { 
...
oldchar.check {
  ...
  if (%current.ctime > %ctime.calc.sixmonths) { 
    ...
    echo -a 04 $+ $1 is older than 6 months and is being removed..
    $zap_char($1)
  }
...
clear_skills {
  ...
  writeini $char($1) skills softblows.on off | writeini $char($1) skills ricochet.on off
...
clear_skill_timers {
  ...
  remini $char($1) skills bribe.time | remini $char($1) skills ricochet.time
}
...
clear_most_status {
  ...
  writeini $char($1) Status bleed no | writeini $char($1) Status HeavyBleed no | writeini $char($1) Status Heavy-Bleed no | writeini $char($1) status bleed-heavy no
  ...
  writeini $char($1) status bleed.timer 0
...
clear_negative_status {
  ...
  writeini $char($1) status poison.timer 0 | writeini $char($1) status bleed.timer 0 |  writeini $char($1) status amnesia.timer 0 | writeini $char($1) status paralysis.timer 0 | writeini $char($1) status drunk.timer 0
...
HQ-Gen-Status {
  set %current.hp $readini($char($1), Battle, HP) | set %max.hp $readini($char($1), BaseStats, HP) | set %hp.percent $calc((%current.hp / %max.hp)*100) |  unset %current.hp | unset %max.hp
  if (%hp.percent > 100) { set %healthbar.char1 $readini(translation.dat, healthbar1, enhanced)  | return }
  if (%hp.percent = 100) { set %healthbar.char1 $readini(translation.dat, healthbar1, 100)  | return }
  if ((%hp.percent < 100) && (%hp.percent >= 90)) { set %healthbar.char1 $readini(translation.dat, healthbar1, 90)  | return }
  if ((%hp.percent < 90) && (%hp.percent >= 80)) { set %healthbar.char1 $readini(translation.dat, healthbar1, 80)  | return }
  if ((%hp.percent < 80) && (%hp.percent >= 70)) { set %healthbar.char1 $readini(translation.dat, healthbar1, 70)  | return }
  if ((%hp.percent < 70) && (%hp.percent >= 60)) { set %healthbar.char1 $readini(translation.dat, healthbar1, 60)  | return }
  if ((%hp.percent < 60) && (%hp.percent >= 50)) { set %healthbar.char1 $readini(translation.dat, healthbar1, 50)  | return }
  if ((%hp.percent < 50) && (%hp.percent >= 40)) { set %healthbar.char1 $readini(translation.dat, healthbar1, 40)  | return }
  if ((%hp.percent < 40) && (%hp.percent >= 30)) { set %healthbar.char1 $readini(translation.dat, healthbar1, 30)  | return }
  if ((%hp.percent < 30) && (%hp.percent >= 20)) { set %healthbar.char1 $readini(translation.dat, healthbar1, 20)  | return }
  if ((%hp.percent < 20) && (%hp.percent >= 0)) { set %healthbar.char1 $readini(translation.dat, healthbar1, 10)  | return }
  if (%hp.percent <= 0)  { set %healthbar.char1 $readini(translation.dat, healthbar1, 0)  | return }
}
...
clear_variables { 
  ...
  unset %hq-gen on | unset %hq-gen.drop.rewards
  unset %nick1 | unset %nick2 | unset %item.winner1 | unset %item.winner2
  unset %powercrystal on  | unset %powercrystal.squad | unset %powercrystal.drop.rewards
  ...
  unset %line | unset %file | unset %name | unset %curbat | unset %real.name | unset %attack.target | unset %killcoin.winners
  ...
  unset %shaken | unset %info.fullbringmsg | unset %basepower | unset %fullbring.needed | unset %poison | unset %totalwins | unset %bleed
  ...
  unset %grand.total | unset %number.of.npcs | unset %doom.timer | unset %monsterfair* | unset %bleed.timer
}
clear_variables2 {
  unset %torment.*
...
give_random_reward {
  if ($readini($txtfile(battle2.txt), battle, bonusitem) != $null) {
    ...
    if (((%battle.type = boss) || (%battle.type = dragonhunt) || (%battle.type = HQ-GEN) || (%battle.type = dungeon))) { var %reward.chance 100 }
    else { 
...
check.clone.death {
  if ($isfile($char($1 $+ _clone)) = $true) { 
    if ($readini($char($1 $+ _clone), battle, status) != dead) { writeini $char($1 $+ _clone) battle status dead | writeini $char($1 $+ _clone) battle hp 0 | $set_chr_name($1 $+ _clone) 
      $display.message(04 $+ %real.name disappears back into $set_chr_name($1) %real.name $+ 's shadow., battle) 
    }
  }
  if ($isfile($char($1 $+ _summon)) = $true) { 
    if ($readini($char($1 $+ _summon), battle, status) != dead) { writeini $char($1 $+ _summon) battle status dead | writeini $char($1 $+ _summon) battle hp 0 | $set_chr_name($1 $+ _summon) 
      $display.message(04 $+ %real.name fades away.,battle) 
    }
  }
}
...
bot.admin {
  if ($1 = list) { var %bot.admins $readini(system.dat, botinfo, bot.owner) 
    if (%bot.admins = $null) { $display.message(04There are no bot admins set., private) | halt }
    else {
      set %replacechar $chr(044) $chr(032)
      %bot.admins = $replace(%bot.admins, $chr(046), %replacechar)
      unset %replacechar
      if ($2 != private) {  $display.message(03Bot Admins:12 %bot.admins, private) }
      if ($2 = private) { $display.private.message(03Bot Admins:12 %bot.admins) }
      halt 
    }
  }

  if ($1 = add) { $checkchar($2) | var %bot.admins $readini(system.dat, botinfo, bot.owner) 
    if ($istok(%bot.admins,$2,46) = $true) { $display.message(04Error: $2 is already a bot admin, private) | halt }
    %bot.admins = $addtok(%bot.admins,$2,46) | $display.message(03 $+ $2 has been added as a bot admin., private) 
    writeini system.dat botinfo bot.owner %bot.admins | halt 
  }

  if ($1 = remove) { var %bot.admins $readini(system.dat, botinfo, bot.owner) 
    if ($istok(%bot.admins,$2,46) = $false) { $display.message(04Error: $2 is not a bot admin, private) | halt }
    ...
    if ($2 = %bot.owner) { $display.message(04Error: $2 cannot be removed from the bot admin list using this command, private) | halt }

    %bot.admins = $remtok(%bot.admins,$2,46) | $display.message(03 $+ $2 has been removed as a bot admin., private) 
    writeini system.dat botinfo bot.owner %bot.admins | halt 
  }
}
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
hq-gen.check {
  ...
  set %hq-gen-chance $round($calc(%HQ-EnergygeneratorHP / %hq-Gen-HP * 100),0)
  set %random.chance-hq $rand(1,100)
  if (%hq-gen-chance >= %random.chance-hq) && ($readini(battlestats.dat, Bounty, BossName) != $null) { $startnormal(hq-gen) | halt }
  if (%hq-gen-chance == 100) { $startnormal(hq-gen) | halt }
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
powercrystal.check {
  var %crystal.power $readini(battlestats.dat, PowerCrystal, Power)
  if (%crystal.power = $null) { var %crystal.power 0 }
  inc %crystal.power $rand(1,5)
  writeini battlestats.dat PowerCrystal Power %crystal.power
  var %spyreport $rand(1,100)
  if (%spyreport >= 50) { $display.message($readini(translation.dat, System, CheckCrystalPower), global) }

  if (%crystal.power >= 100) { 
    $startnormal(pcrystalrun)
    writeini battlestats.dat PowerCrystal Power 0
    halt
  }
}
...
system.intromessage {
  ...
  if (%current.month = 02) { $display.private.message(07*02 $readini(translation.dat, EventInfo, February)) }
  if (%current.month = 03) { $display.private.message(07*02 $readini(translation.dat, EventInfo, March)) }
  if (%current.month = 04) { $display.private.message(07*02 $readini(translation.dat, EventInfo, April)) }
  if (%current.month = 07) { $display.private.message(07*02 $readini(translation.dat, EventInfo, July)) }
  if (%current.month = 10) { $display.private.message(07*02 $readini(translation.dat, EventInfo, October)) }
  if (%current.month = 11) { $display.private.message(07*02 $readini(translation.dat, EventInfo, November)) }
  if (%current.month = 12) { $display.private.message(07*02 $readini(translation.dat, EventInfo, December)) }
  ...
  if (%ignore.dailylogin != true) {   
    ...
    $display.private.message(02[12Login Bonus02] For logging into the game today you have received04 %reward.amount 02x04 %reward.type $+ 02!)
  }
  ...
  if (%resting.bonus.orbs > 1) {
    $display.private.message(02[12Resting Bonus02] In your next battle you will receive a bonus of04 $bytes(%resting.bonus.orbs,b) 02 $+ $return.systemsetting(currency) and04 $bytes(%resting.bonus.killcoins,b) 02killcoins)
  }
...
bounty.display {
  if ($readini(battlestats.dat, Bounty, BossName) = $null) { $display.message(04There is no current bounty, private) | halt }
  else {
