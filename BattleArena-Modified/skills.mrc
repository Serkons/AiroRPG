...
alias use.skill { 
  ...
  if ($3 = taming) { $skill.taming($1) }
  if ($3 = ThrowPoke) { $skill.ThrowPoke($1) }
...
alias fullbring.aoestatus {
  ...
  if (%fullbring.status = bleed) { set %status.type bleed | var %tech.status.grammar bleed to death }
  ...
  while (%battletxt.current.line <= %battletxt.lines) { 
    ...
    if ($readini($char(%who.battle), info, flag) != monster) { inc %battletxt.current.line }
      ...
      if (%current.status != dead) { 
        ...
        if (%chance >= 50) {
          if ((%chance = 70) && (%fullbring.status = bleed)) { $set_chr_name(%who.battle) | set %statusmessage.display 04 $+ %real.name is now %tech.status.grammar $+ !  | writeini $char(%who.battle) Status bleed-heavy yes }
          if ((%chance = 70) && (%fullbring.status != bleed)) { $set_chr_name(%who.battle) | set %statusmessage.display 04 $+ %real.name is now %tech.status.grammar $+ !  | writeini $char(%who.battle) Status %fullbring.status yes }
