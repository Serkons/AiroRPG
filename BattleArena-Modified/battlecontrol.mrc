...
on 1:TEXT:!Theconquest*:*: { $conquest.display($1, $2) }
...
alias beute { 
  var %numdrops $numtok($readini($char(%who), Stuff, Drops),46)
  var %drops $readini($char(%who), Stuff, Drops)
  var %dropnew $replace(%drops,.,$chr(32))
  if (%drops != $null) { return 3[ Has a total of 11 %numdrops 3 Drops: 7 %dropnew 3] }
  if (%drops == $null) { return 3[ Has no Drops 3] }
}
...
alias battle.stats {
  ...
  var %total.pcrystal $readini(battlestats.dat, Battle, powercrystalTimes)
  var %total.pcrystal.wins $readini(battlestats.dat, Battle, TotalpowercrystalWon)
  var %total.pcrystal.fails $readini(battlestats.dat, Battle, TotalpowercrystalLost)
  ...
  $display.message($readini(translation.dat, system, BattleStatsPinfo), private)
...
on 3:TEXT:!show battle save*:*:{ $set_chr_name($nick) | $checkchar($nick) 
...
off 3:TEXT:!save battle level*:*:{   $save.battle.streak($nick, $1, $2, $3) }
...
on 3:TEXT:!reset battle level*:*:{  $reset.battle.streak($nick) } 
alias reset.battle.streak {
  ...
  if (%last.reloadby != $1) { $display.message(04Only a person who reloads a battle save can reset the streak using this command) | halt }
  ...
  $display.message(03The winning streak has been reset to: 0, global)
}

on 3:TEXT:!reload battle level*:*:{  $reload.battle.streak($nick, $1, $2, $3) } 
...
on 50:TEXT:!clear battle stats*:*:{ 
  ...
  writeini battlestats.dat battle PcrystalRuns 0
  writeini battlestats.dat battle PcrystalRuns.Failed 0
  writeini battlestats.dat battle PcrystalRuns.Won 0
...
alias clear_battle { 
  ...
  var %people.in.battle $readini($txtfile(battle2.txt), Battle, List)
  if (($istok(%people.in.battle,bark_spider,46) = $true) && ($1 = victory)) { 

    if (((%battle.type != cosmic) && (%battle.type != MonsterFair) && (%battle.type != torment)))  {
      if ($isfile($dungeonfile(Rakna-Kadaki-cave)) = $true) { 
        ; a 25% chance of this happening
        set %rakna $rand(1,100)
        if (%rakna <= 25) { var %open.treasuredungeon rakna }
      }
    }
  }
  ...
  ;$system.start.newbattle(%open.raknadungeon)
...
alias startnormal { 
  ...
  if (%start.battle.type != ai) {
    ...
    if (%start.battle.type = pcrystalrun) { 
      set %pcrystalrun on
      $display.message($readini(translation.dat, Events, powercrystal), global)
      var %powercrystal.times $readini(battlestats.dat, battle, powercrystalTimes)
      if (%powercrystal.times = $null) { var %powercrystal.times 0 }
      inc %powercrystal.times 1 
      writeini battlestats.dat battle powercrystalTimes %powercrystal.times
      set %battle.type boss
    }

    if (%start.battle.type = hq-gen) {
      set %hq-gen on
      var %boss.name $readini(battlestats.dat, Bounty, BossName)
      $display.message($readini(translation.dat, Events, hq-gen), global)
      set %battle.type HQ-Gen
    }

    if ((((((%start.battle.type = boss) && ($2 != supplyrun) && ($2 != hq-gen) && ($2 != pcrystalrun) && ($2 != besieged) && ($2 != savethepresident)))))) { 
      set %battle.type boss 
    ...
    var %valid.battle.types ai.boss.monster.orbbattle.orbfountain.orb_fountain.pvp.gauntlet.manual.mimic.defendoutpost.assault.dragonhunt.torment.pcrystalrun.cosmic.hq-gen.besieged.supplyrun.monsterfair
...
; ==========================
on 3:TEXT:!enter:#: {
  var %current.hp $readini($char($nick), BaseStats, HP)
  var %current.wp $readini($char($nick), Weapons, Equipped)
  ...
  if (%tut == on) && (%current.hp <= 200) && ($current.wp != fists) { msg $chan Sorry $nick it is a risk with no weapon and less than 200 hp to battle. Please skill your char first | halt }
  else { $enter($nick) }
}
...
alias enter {
  ...
  if ((%battle.type = defendoutpost) || (%battle.type = assault)) { 
    ...
    if ($return_winningstreak >= 100) { 
      ...
      if ($get.level($1) > $calc(3 + $return_winningstreak)) { 
        ...
        $display.message(04 $+ %real.name has been synced to level $calc(3 + $return_winningstreak) for this battle, battle)
      }
    }
  }
  ...
  if (%supplyrun = on) { var %battle.level.cap $return_winningstreak
    if ($get.level($1) > %battle.level.cap) {  $levelsync($1, $calc(%battle.level.cap + 5)) | $display.message(04 $+ %real.name has been synced to level $calc(%battle.level.cap +5) for this battle, battle) }
  }
  ...
  ; Level sync the Crystal battles
  if (%pcrystalrun = on) { var %battle.level.cap $calc($return_winningstreak +2)
    if ($get.level($1) > %battle.level.cap) {  $levelsync($1, %battle.level.cap) | $display.message(04 $+ %real.name has been synced to level %battle.level.cap for this battle, battle) }
  }
...
alias battlebegin {
  ...
  if (%battle.type != mimic) { 
    if ((((%boss.type != bandits) && (%boss.type != pirates) && (%boss.type != zombies) && (%boss.type != FrostLegion)))) { 
      unset %current.battlefield 
  ...
  if (%pcrystalrun = on) { set %current.battlefield Allied Forces PowerCrystal Chamber }
  if (%hq-gen = on) { set %current.battlefield Allied Forces Energy Center }
  ...
  if ((%battle.type != ai) && (%besieged != on) || (%pcrystalrun != on) || (%hq-gen != on)) {
  ...
  if (%pcrystalrun = on) { set %darkness.turns 110 | writeini battlestats.dat PowerCrystal Power 0 }
  if (%hq-gen = on) { set %darkness.turns 120 }
  ...
  if (%besieged = on) {  $display.message(04The first squad of monsters has arrived!, battle)  }
  if (%powercrystal = on) { $display.message(4The First Super Elite Squad of monsters has arrived!, battle) }
  ...
  if (%hq-gen = on) { $display.message($readini(translation.dat, battle, hq-genGoal), battle) }
  if (%pcrystalrun = on) { $display.message($readini(translation.dat, battle, PowercrystalGoal), battle) } 
...
alias battle.getmonsters {
  ...
  if (%hq-gen = on) { $generate_hq-gen | $generate_hq-gen-boss }
  if (%powercrystal = on) { $generate_monster(powercrystal) }
  if (%battle.type = monsterfair) { $monsterfair.getmonsters | return }

  if ((%mode.pvp != on) && (%besieged != on) || (%powercrystal != on) || (%hq-gen != on)) {
      ...
      if (%battle.type != manual) { 
        if (%winning.streak >= 101) { 
          if (%demonwall.fight != on) {
            ...
            if (%boss.type = zombies) { set %number.of.monsters.needed 0 }
            ...
            if (%boss.type = HQ-Gen) { set %number.of.monsters.needed 0 }

            if ((%supplyrun = on) && ($return_winningstreak > 500)) {  set %number.of.monsters.needed 1  }
            if (%hq-gen = on) { set %number.of.monsters.needed 0 }
            if ((%pcrystalrun = on) && ($return_winningstreak < 500)) {  set %number.of.monsters.needed $rand(3,6) | set %mon.need %number.of.monsters.needed }
...
alias generate_monster {
  ...
  if ($1 = hq-gen) {
    ...
    .copy -o $npc(HQ-Generator) $char(HQ-Generator)  | set %curbat $readini($txtfile(battle2.txt), Battle, List) | %curbat = $addtok(%curbat,HQ-Generator,46) | writeini $txtfile(battle2.txt) Battle List %curbat | write $txtfile(battle.txt) HQ-Generator
    $boost_monster_stats(HQ-Generator) | $fulls(HQ-Generator)
    var %battlemonsters $readini($txtfile(battle2.txt), BattleInfo, Monsters) | inc %battlemonsters 1 | writeini $txtfile(battle2.txt) BattleInfo Monsters %battlemonsters
    $set_chr_name(HQ-Generator) 
    $display.message($readini(translation.dat, battle, EnteredTheBattle), battle)
    $display.message(12 $+ %real.name  $+ $readini($char(HQ-Generator), descriptions, char), battle)
  ...
  if ($1 = hq-gen-boss) {
    ...
    var %monster.level 1
    if (%battle.type = HQ-Gen) { set %number.of.monsters.needed 0 }
    set %bountyboss $readini(battlestats.dat, Bounty, BossName)
    if ($readini(battlestats.dat, HQ-Gen, Failed) == $null) { var %monster.level $calc(1000 * 1) }
    if ($readini(battlestats.dat, HQ-Gen, Failed) != $null) { var %power.powercore $readini(battlestats.dat, HQ-Gen, Failed) | var %monster.level $calc($current.battlestreak * 1000 + %power.powercore) }

    .copy -o $boss(%bountyboss) $char(%bountyboss)
    var %battlemonsters $readini($txtfile(battle2.txt), BattleInfo, Monsters) 
    set %curbat $readini($txtfile(battle2.txt), Battle, List) |  %curbat = $addtok(%curbat,%bountyboss,46) |  writeini $txtfile(battle2.txt) Battle List %curbat  |   $set_chr_name(%bountyboss) 
    write $txtfile(battle.txt) %bountyboss 
    var %battlemonsters $readini($txtfile(battle2.txt), BattleInfo, Monsters) | inc %battlemonsters 1 | writeini $txtfile(battle2.txt) BattleInfo Monsters %battlemonsters 

    ; Display the entering message and quote
    $display.message($readini(translation.dat, battle, EnteredTheBattle), battle)
    $display.message(12 $+ %real.name  $+ $readini($char(%bountyboss), descriptions, char), battle)
    if ($readini($char(%bountyboss), descriptions, BossQuote) != $null) { $display.message(02 $+ %real.name looks at the heroes and says " $+ $readini($char(%bountyboss), descriptions, BossQuote) $+ ", battle) }
    writeini $char(%bountyboss) battle status normal 

    ; Boost the monster
    $levelsync(%bountyboss, $calc(1000 + %power.powercore))
    ;writeini $char(%bountyboss) basestats str $readini($char(%bountyboss), battle, str)
    ; writeini $char(%bountyboss) basestats def $readini($char(%bountyboss), battle, def)
    ;writeini $char(%bountyboss) basestats int $readini($char(%bountyboss), battle, int)
    ;writeini $char(%bountyboss) basestats spd $readini($char(%bountyboss), battle, spd)
    $boost_monste_stats(%bountyboss, $calc($current.battlestreak * 1000 + %power.powercore))
    $fulls(%bountyboss)
    return
  ...
  if ($1 = powercrystal) {
    if (%powercrystal.squad = 1) { var %number.of.monsters.needed = $rand(1,15) | var %monster.line.file monster.lst | var %monster.level 70 }
    if (%powercrystal.squad = 2) { var %number.of.monsters.needed = $rand(1,10) | var %monster.line.file monster.lst | var %monster.level 90 }
    if (%powercrystal.squad = 3) { var %number.of.monsters.needed = $rand(1,8) | var %monster.line.file monster.lst | var %monster.level 100 }
    if (%powercrystal.squad = 4) { var %number.of.monsters.needed = $rand(1,5) | var %monster.line.file monster.lst | var %monster.level 110 }
    if (%powercrystal.squad = 5) { var %number.of.monsters.needed = $rand(1,4) | var %monster.line.file monster.lst | var %monster.level 120 }
    ...
    while (%value <= %number.of.monsters.needed) {
      ...
      if ($isfile($boss(%current.monster.to.spawn)) = $true) { .copy -o $boss(%current.monster.to.spawn) $char(%current.monster.to.spawn.name)  }
      if ($isfile($mon(%current.monster.to.spawn)) = $true) {  .copy -o $mon(%current.monster.to.spawn) $char(%current.monster.to.spawn.name) | set %elitearmor $round($calc($readini($char(%current.monster.to.spawn.name), BaseStats, HP) * 3 + $rand(1, 100)), 0) | set %armorvalue $rand(1, %elitearmor) | writeini $char(%current.monster.to.spawn.name) NaturalArmor Name EliteArmor | writeini $char(%current.monster.to.spawn.name) NaturalArmor Max %armorvalue | writeini $char(%current.monster.to.spawn.name) NaturalArmor Current %armorvalue }
  ...
  if ($1 = monster) {
    ...
    while (%value <= %number.of.monsters.needed) {
      ...
      var %chr_value  $readini($char(%monster.name), BaseStats, Name)
      var %batfield %current.battlefield
      if (%batfield == graveyard) { writeini $char(%monster.name) Status ethereal yes }
      if (%battle.type != ai) { 
        if ($readini($char(%monster.name), battle, hp) != $null) && (SuperElite !isin %monster.name) || (Armored !isin %monster.name) { 
          $display.message($readini(translation.dat, battle, EnteredTheBattle), battle)
  ...
  if ($1 = boss) {
    ...
    set %valid.boss.types normal.bandits.gremlins.doppelganger.warmachine.demonwall.wallofflesh.predator.zombies.pirates.frostlegion.crystalshadow.dinosaurs
    ...
    if (%battle.type = cosmic) { 
      ...
      if ($isfile($boss(%monster.name)) = $null) { $display.message(04Error: Boss not found! Ending Now!) | $endbat | halt }
      ...
      $display.message($readini(translation.dat, battle, EnteredTheBattle), battle)
      $display.message(12 $+ %real.name  $+ $readini($char(%monster.name), descriptions, char), battle)
      if ($readini($char(%monster.name), descriptions, BossQuote) != $null) { $display.message(02 $+ %real.name looks at the heroes and says " $+ $readini($char(%monster.name), descriptions, BossQuote) $+ ", battle) }
    ...
    if (%boss.type = normal) {
      ...
      if ((%monsters.total = 0) || (%monster.list = $null)) { $display.message(04Error: There are no bosses in the boss folder.. Have the bot admin check to make sure there are bosses for players to battle!, global) | $endbattle(none) | halt }
      if (%mode.gauntlet != $null) { set %number.of.monsters.needed 2  }
      if (%battle.type = defendoutpost) { set %number.of.monsters.needed 1 }
      if (%batte.type = HQ-Gen) { set %number.of.monsters.needed 0 }
    ...
    if (%boss.type = gremlins) {
      ...
      if (%battle.type != ai) { 
        ...
        $display.message(02 $+ %real.name looks at the heroes and says " $+ $readini($char(stripe), descriptions, BossQuote) $+ ", battle)
      }
    ...
    if (%boss.type = zombies) {
      $display.message($readini(translation.dat, events, ZombieFight), battle)

      if (%battle.type != ai) { 
        var %battle.level.cap $return.levelcapsetting(Zombies)
        if (%battle.level.cap = null) { var %battle.level.cap 85 } 
        $display.message($readini(translation.dat, system,BossLevelSyncedCustom), battle)
        $portal.sync.players(%battle.level.cap) 
      }
      ...
      if ($readini(battlestats.dat, Battle, WinningStreak) >= 100) {  var %number.of.zombies.needed $rand(1,4) }
      else {  var %number.of.zombies.needed $rand(1,2) }
      var %i 1
      while (%i <= %number.of.zombies.needed) {
        $generate_zombie_minion(%i)
        inc %i 1
      }

      set %current.battlefield swamp
    }
...
alias generate_battle_order {
  ...
  if (%current.turn > %max.demonwall.turns) { 
    if (%demonwall.fight = on) { 
      ...
      if (%battle.type = ai) { set %ai.winner 02Winner:04 %ai.monster.name | writeini $txtfile(1vs1bet.txt) money winner monster  } 
      /endbattle defeat 
      halt 
    }
  }
...
alias battle.end.failure {
  ...
  if (((((((((%portal.bonus != true) && (%battle.type != cosmic) && (%battle.type != torment) && (%battle.type != monsterfair) && (%battle.type != mimic) && (%battle.type != dragonhunt) && (%supplyrun != on) && (%pcrystalrun != on) && (%savethepresident != on))))))))) {
  ...
  if (%hq-gen = on) {
    var %bountyboss $readini(battlestats.dat, Bounty, BossName)
    $display.message($readini(translation.dat, battle, EvilHasWonGen), Global)
    var %total.hq-gen.failed $readini(battlestats.dat, HQ-Gen, Failed)
    if (%total.hq-gen.failed = $null) { var %total.hq-gen.failed 0 }
    inc %total.hq-gen.failed 1
    writeini battlestats.dat HQ-Gen Failed %total.hq-gen.failed
    $conquest.points(players,-1000) |  $conquest.points(monsters,500) | $shopnpc.kidnap | $shopnpc.kidnap
    writeini $npc(HQ-Generator) Battle HP 0
  }

  if (%pcrystalrun = on) {
    $display.message($readini(translation.dat, battle, EvilHasWonPowerCrystal), global) 
    var %total.powercrystal $readini(battlestats.dat, battle, TotalpowercrystalLost)
    if (%total.powercrystal = $null) { var %total.powercrystal 0 }
    inc %total.powercrystal 1
    writeini battlestats.dat battle TotalpowercrystalLost %total.powercrystal 
    $conquest.points(players,-1000) |  $conquest.points(monsters,500) | $shopnpc.kidnap | $shopnpc.kidnap
  } 
...
alias battle.end.victory {
  ...
  if (((((((((((((%portal.bonus != true) && (%battle.type != cosmic) && (%battle.type != powercrystal) && (%battle.type != torment) && (%battle.type != monsterfair) && (%besieged != on) && (%battle.type != mimic) && (%battle.type != defendoutpost) && (%battle.type != assault) && (%battle.type != dragonhunt) && (%supplyrun != on) && (%pcrystalrun != on) && (%hq-gen != on) && (%savethepresident != on))))))))))))) {
  ...
  if (%hq-gen = on) {
    var %bountyboss $readini(battlestats.dat, Bounty, BossName)
    $display.message($readini(translation.dat, battle, GoodHasWonGen), Global)
    var %total.hq-gen.success $readini(battlestats.dat, HQ-Gen, Success)
    var %total-hq-gen.failed $readini($battlestats.dat, HQ-Gen, Failed)
    if (%total.hq-gen.success = $null) { var %total.hq-gen.success 0 }
    if (%total-hq-gen.failed = $null) { var %total.hq-gen.failed 0 }
    inc %total.hq-gen.success 1
    dec %total.hq-gen.failed %total.hq-gen.success
    writeini battlestats.dat HQ-Gen Success %total.hq-gen.Success
    $conquest.points(players,1000) |  $conquest.points(monsters,-1000) 
    writeini $npc(HQ-Generator) Battle HP 0
  }

  if (%pcrystalrun = on) {
    $display.message($readini(translation.dat, battle, GoodHasWonpowercrystal), global) 
    var %total.powercrystal $readini(battlestats.dat, battle, TotalpowercrystalWon)
    if (%total.powercrystal = $null) { var %total.powercrystal 0 }
    inc %total.powercrystal 1
    writeini battlestats.dat battle TotalpowercrystalWon %total.powercrystal 
    $conquest.points(monsters,-1000) | $conquest.points(players,1000)
  } 
  ...
  $display.message($readini(translation.dat, battle, RewardKillCoinsWin), battle)
  ; Check to see if allied notes are involved.
  if (((((%portal.bonus = true) || (%savethepresident = on) || (%supplyrun = on) || (%pcrystalrun = on) || ($readini($txtfile(battle2.txt), battle, alliednotes) != $null))))) { $display.message($readini(translation.dat, battle, AlliedNotesGain), battle)  }

  ; Check for Allied Influence
  if (%supplyrun != on) { $battle.reward.influence(alliedforces) }
  if (%pcrystalrun != on) { $battle.reward.influence(alliedforces) }
  ; If boss battle, do black orbs for select players.
  unset %black.orb.winners
  if (%battle.type = monster) {
    set %atchance $rand(1,100)
    if (%atchance <= 25) {
      var %an 3At the end of this Battle an 7Armor Chest3 appers and was Automatically Openend and 1 or 2 Random players gets the content.
      $display.message(%an, battle)
      $give_armor_chest
    }
  }

  if (((((%battle.type = boss) || (%battle.type = assault) || (%battle.type = HQ-Gen) || (%battle.type = dragonhunt) || (%battle.type = defendoutpost))))) {
  ...
  if (%pcrystalrun = on) { $powercrystal .reward } 
  if (%hq-gen = on) { $hq-gen.reward }
...
alias turn {
  ...
  if ($return.systemsetting(TurnType) = action) {
    ...
    else { set %status.message $readini(translation.dat, battle, TurnMessage) | set %status.message2 $readini(translation.dat, battle, TurnMessage2) | set %status.message3 $readini(translation.dat, battle, TurnMessage3) | set %status.message4 $readini(translation.dat, battle, TurnMessage4) }
  }
  else { set %status.message $readini(translation.dat, battle, TurnMessage) | set %status.message2 $readini(translation.dat, battle, TurnMessage2) | set %status.message3 $readini(translation.dat, battle, TurnMessage3) | set %status.message4 $readini(translation.dat, battle, TurnMessage4) }
  ...
  if (%hq-gen = on) { $hq-Gen-Status(HQ-Generator) | set %m1 $readini(Translation.dat, battle, TurnMessageHQ-Gen) | $display.message.delay(%m1, battle, 1) }

  $display.message.delay(%status.message, battle, 1)
  $display.message.delay(%status.message2, battle, 1)
  $display.message.delay(%status.message3, battle, 1)
  $display.message.delay(%status.message4, battle, 1)
...
alias naturalarmor {
  if ($readini($char(%who), NaturalArmor, Current) <= 0) || ($readini($char(%who), NaturalArmor, Current) == $null) { return 7No }
  if ($readini($char(%who), NaturalArmor, Current) >= 1) || ($readini($char(%who), NaturalArmor, Current) != $null) { return 7 $readini($char(%who), NaturalArmor, Name) - $readini($char(%who), NaturalArmor, Current) / $readini($char(%who), NaturalArmor, Max) }
}
...
alias battle.check.for.end {
  ...
  if ((%pcrystalrun = on) && ($readini($char(powercrystal), battle, hp) <= 0)) { /.timerEndBattle $+ $rand(a,z) 1 4 /endbattle defeat | halt  }
  if ((%hq-gen = on) && ($readini($char(HQ-Generator), battle, hp) <= 0)) { /.timerEndBattle $+ $rand(a,z) 1 4 /endbattle defeat | halt  }
  ...
  if (%battle.type != dungeon) { 
    if ((%battle.monster.death = true) && (%battle.player.death = false)) { 
      if (%battle.type = ai) { set %ai.winner 02Winner:12 %ai.npc.name | writeini $txtfile(1vs1bet.txt) money winner npc  } 
...
alias battlelist { 
  ...
  while (%l <= %lines) { 
    ...
    if (%status.battle = $null) { inc %l 1 }
      ...
      if ((%status.battle = dead) || (%status.battle = runaway)) { 
        var %token.to.add 04 $+ %who.battle
        ...
        if ($readini($char(%who.battle), info, flag) = $null) { 
          if ($return.systemsetting(TurnType) = action) { var %action.points $chr(91) $+ $action.points(%who.battle, check) $+ $chr(93) }
          var %token.to.add 03 $+ %who.battle $+ %action.points
        }
  ...
  if ($1 = $null) {
    ...
    if (%battleconditions != $null) { var %batlist.battleconditions [Conditions:12 $replace(%battleconditions, $chr(046), $chr(044) $chr(032)) $+ 04] } 

    $display.message($readini(translation.dat, battle, BatListTitleMessage), private)
    $display.message(04[Turn #:12 %current.turn $+ 04] [Weather:12 $readini($dbfile(battlefields.db), weather, current) $+ 04] [Moon Phase:12 %moon.phase $+ 04] [Time of Day:12 $readini($dbfile(battlefields.db), TimeOfDay, CurrentTimeOfDay) $+ 04] [Battlefield:12 %current.battlefield $+ 04] 4 [ You can scavenge for 7 $replace($readini($dbfile(battlefields.db), %current.battlefield, Scavenge),., $chr(32)) 4] %batlist.battleconditions , private)

    if ((%darkness.turns != $null) && (%battle.type != ai)) { 
      var %darkness.countdown $calc(%darkness.turns - %current.turn) 
      if (%battle.type = defendoutpost) {  
        if (%darkness.turns > 0) { $display.message(04[The Outpost will be defended in:12 %darkness.turns 04waves], private) }
      }
      if (%battle.type != defendoutpost)  {
        if (%darkness.countdown > 0) { $display.message(04[Darkness will occur in:12 %darkness.countdown 04turns], private) }
        if (%darkness.countdown <= 0) { $display.message(04[Darkness12 has overcome 04the battlefield], private) }
      }

      if (%battle.type = assault) {  $display.message(04[Outpost Strength: $monster.outpost(strengthbar) $+ 04], private) }

    }

    if (%battle.type != ai) { 
      $display.message(04[Battle Order: %battle.list $+ 04], private) 
    }
    ...
    if (%battle.type = ai) { 
      ...
      $display.message(04[Total Betting Amount:12 %currency.symbol $+ $bytes(%money.bet,b) $+ 04] [Odds:12 %odds.npc $+ $chr(58) $+ %odds.monster $+ 04], private)
    }
  ...
  if ($1 = public) { 
    ...
    $display.message(04[Turn #:12 %current.turn $+ 04][Weather:12 $readini($dbfile(battlefields.db), weather, current) $+ 04] [Moon Phase:12 %moon.phase $+ 04] [Time of Day:12 $readini($dbfile(battlefields.db), TimeOfDay, CurrentTimeOfDay) $+ 04] [Battlefield:12 %current.battlefield $+ 04] 3[ You can scavenge for 7 $replace($readini($dbfile(battlefields.db), %current.battlefield, Scavenge),., $chr(32)) ] %batlist.battleconditions, battle)

    if ((%darkness.turns != $null) && (%battle.type != ai)) { 
      var %darkness.countdown $calc(%darkness.turns - %current.turn) 

      if (%battle.type = defendoutpost) {  
        if (%darkness.turns > 0) { $display.message(04[The Outpost will be defended in:12 %darkness.turns 04waves], battle) }
      } 
      if (%battle.type != defendoutpost)  {
        if (%darkness.countdown > 0) { $display.message(04[Darkness will occur in:12 %darkness.countdown 04turns], battle) }
        if (%darkness.countdown <= 0) { $display.message(04[Darkness12 has overcome 04the battlefield], battle) }
      }

      if (%battle.type = assault) {  $display.message(04[Outpost Strength: $monster.outpost(strengthbar) $+ 04], battle) }
    }

    if (%battle.type != ai) { 
      $display.message(04[Battle Order: %battle.list $+ 04], battle) 
    }
    ...
    if (%battle.type = ai) { 
      ...
      var %currency.symbol $readini(system.dat, system, BetCurrency)
      if (%currency.symbol = $null) { var %currency.symbol $chr(36) $+ $chr(36) }
      $display.message(04[Total Betting Amount:12 %currency.symbol $+ $bytes(%money.bet,b)  $+ 04] [Odds:12 %odds.npc $+ $chr(58) $+ %odds.monster $+ 04], battle)
    }
...
alias battle.calculate.redorbs {
  ...
  if (((((%boss.type = bandits) || (%boss.type = pirates) || (%boss.type = zombies) || (%boss.type = FrostLegion) || (%boss.type = gremlins))))) { inc %number.of.monsters.needed 2 }
...
alias battle.reward.redorbs {
  ...
  while (%battletxt.current.line <= %battletxt.lines) { 
    ...
    if ((%flag = monster) || (%flag = npc)) { inc %battletxt.current.line 1 }
      ...
      ; Check for the "First Kill" achievement courtesy of Zenturion
      if (%battle.type = monster) || (%battle.type = orbfountain) {
        if (($1 = victory) && ($readini($char(%who.battle), battle, status) = alive)) { $achievement_check(%who.battle, FirstKill) }
      }
  ...
  if ($chr(046) isin %killcoin.winners) { set %replacechar $chr(044) $chr(032)
    %killcoin.winners = $replace(%killcoin.winners, $chr(046), %replacechar)
  }
}

On *:text:!z:#: { $achievement_check(Zentaurus, FirstKill) }
...
alias battle.reward.killcoins {
  ...
  while (%battletxt.current.line <= %battletxt.lines) { 
    ...
    if ((%flag = monster) || (%flag = npc)) { inc %battletxt.current.line 1 }
      ...
      ;Added to the %red.orb.winners variable
      %killcoin.winners = $addtok(%killcoin.winners, 3 $+ %who.battle 3 $+ $chr(91) $+ $chr(43) $+ %total.coins.reward $+ $chr(93),46)
...
alias turn.statuscheck {
  ...
  $doom_check($1) | $poison_check($1) | $bleed_check($1) | $zombie_check($1) | $zombieregenerating_check($1) | $doll_check($1) | $virus_check($1)  | $silence_check($1)
